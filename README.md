...
- **guacd FreeRDP 3.25 callback ABI fix (v1.3.2)** — FreeRDP 3.25 deleted the legacy `Authenticate` callback field on `struct rdp_freerdp` in favour of a new five-argument `AuthenticateEx` that takes an additional `rdp_auth_reason reason` parameter. The moment Alpine edge bumped `freerdp-dev` from `3.24.2-r0` to `3.25.0-r0`, the custom guacd image stopped compiling with `'struct rdp_freerdp' has no member named 'Authenticate'; did you mean 'AuthenticateEx'?`. New patch [`guacd/patches/006-freerdp325-authenticate-ex.patch`](guacd/patches/006-freerdp325-authenticate-ex.patch) adds an explicit `#include <freerdp/version.h>` near the existing `<freerdp/...>` includes in `rdp.c` so the `FREERDP_VERSION_MAJOR` / `FREERDP_VERSION_MINOR` macros are visible at the conditional, then `#if`-selects the new five-argument `AuthenticateEx` signature on FreeRDP ≥ 3.25 while keeping the four-argument legacy signature on 3.24 and earlier. The added `reason` argument is intentionally discarded with `(void) reason;` because the existing implementation already requests whichever credentials are missing, regardless of the reason FreeRDP raised the callback. The Dockerfile now also runs two `grep -q` assertions immediately after the patch loop that fail the build with a clear error if `#include <freerdp/version.h>` or `rdp_inst->AuthenticateEx = rdp_freerdp_authenticate;` is missing — defence-in-depth catch for silent semantic regressions of future patch refactors. A new [`guacd/patches/.gitattributes`](guacd/patches/.gitattributes) pins `*.patch` to `text eol=lf` so contributors with `core.autocrlf=true` on Windows cannot accidentally introduce CRLF that misaligns hunk context lines.
- **RDP resize ghost-region fix (v1.3.2)** — Reinstated [`guacd/patches/005-refresh-rect-on-resize.patch`](guacd/patches/005-refresh-rect-on-resize.patch). When a Windows RDP server changed resolution mid-session (resolution change inside a VM, GFX channel renegotiation, multi-monitor reconfiguration), the Strata canvas would render a solid-black margin along the edge of the new desktop area until the user moved a window across the affected region to force a repaint. Patch 005 marks the entire layer dirty after `gdi_resize` and asks the RDP server to re-send pixels for the full new desktop area via `context->update->RefreshRect`, with bounds-checks against `UINT16_MAX` so a pathological resize cannot produce a malformed PDU. Two new structured debug log lines (`[strata] guac_rdp_gdi_desktop_resize: resizing %dx%d` and `[strata] post-resize RefreshRect %ux%u -> %s`) make the path observable at `GUAC_LOG_DEBUG`.
- **WebSocket-tunnel auth watchdog (v1.3.2)** — `backend/src/routes/tunnel.rs` `ws_tunnel` now decodes the access token's `exp` claim once at upgrade time (with a fully-validated `Validation::new(Algorithm::HS256)`, `iss=strata-local`, `set_required_spec_claims(&["exp"])`) and runs a 30-second `tokio::time::interval` tick loop alongside the proxy that polls `services::token_revocation::is_revoked(token)` and compares `Utc::now()` to the cached `exp`. Either condition aborts the proxy loop, so the recording flushes and the `session_registry` row decrements within at most one tick — closes the long tail of `ghost recording grows for hours after the user's tab is killed by the OS` reports. Cadence chosen so an aggressive 1-minute access-token TTL still detects revocation in ≤ 30 s while a normal 20-minute TTL costs at most 40 ticks per session — negligible next to the WebSocket I/O. Non-local OIDC tokens cleanly fall through to revocation-only checks.
- **Logout closes tunnels immediately (v1.3.2)** — `App.tsx`'s `handleLogout` (and the idle-timeout path) now calls a new module-level `closeAllSessionsExternal()` helper exported from `SessionManager.tsx` *before* flipping React auth state, then issues a fire-and-forget `apiLogout()` to invalidate the refresh token and clear the auth cookies. The helper iterates every active session and runs the same teardown the per-session disconnect button uses (`cleanupPopout`, `cleanupMultiMonitor`, `_cleanupPaste`, keyboard reset, `client.disconnect()`), each step wrapped in best-effort `try / catch` so a single failure cannot block the rest of the logout. Backend now sees clean WebSocket closes the moment the user clicks **Log out** rather than after the next watchdog tick.
