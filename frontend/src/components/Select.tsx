...
export default function Select({ value, onChange, options, placeholder, disabled, className, searchable }: SelectProps) {
      ...
      {open && createPortal(
        <ul className="cs-menu" role="listbox" ref={menuRef} style={menuStyle}>
          {searchable && (
            <li className="p-2 border-b border-white/5 sticky top-0 bg-surface-secondary/95 backdrop-blur-md z-10">
              <div className="relative">
                ...
                />
                <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {filteredOptions.length === 0 ? (
            <li className="px-3 py-4 text-xs italic text-center opacity-40">No matching options</li>
          ) : (
