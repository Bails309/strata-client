...
-- Create case-insensitive unique indexes (IF NOT EXISTS for idempotent re-run)
CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email_ci    ON users (LOWER(email))    WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_users_username_ci ON users (LOWER(username)) WHERE deleted_at IS NULL;
