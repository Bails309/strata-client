...
export default function FileBrowser({ filesystem, onClose }: Props) {
  ...
  const loadDirectory = useCallback((dirPath: string[]) => {
    ...
    filesystem.object.requestInputStream(streamPath, (stream, mimetype) => {
      ...
      if (mimetype === 'application/vnd.glyptodon.guacamole.stream-index+json') {
        ...
        let json = '';
        reader.ontext = (text: string) => { json += text; };
        reader.onend = () => {
          try {
  ...
  const handleUpload = useCallback((files: FileList) => {
    for (let i = 0; i < files.length; i++) {
      ...
      const stream = filesystem.object.createOutputStream(file.type || 'application/octet-stream', uploadPath);
        ...
        {loading ? (
          ...
          entries.map((entry) => (
            <div
              ...
              title={entry.type === 'directory' ? 'Double-click to open' : 'Double-click to download'}
            >
              {entry.type === 'directory' ? (
                ...
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-txt-tertiary)" strokeWidth="2">
          ...
          {uploads.map((u) => (
            <div key={u.name} style={{ fontSize: '0.7rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                <span className="text-txt-secondary" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '80%' }}>
                  {u.name}
                </span>
                <span>{u.progress}%</span>
              </div>
              <div style={{ height: 3, borderRadius: 2, background: 'var(--color-border)', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${u.progress}%`, background: 'var(--color-accent)', transition: 'width 0.2s' }} />
              </div>
            </div>
          ))}
        </div>
