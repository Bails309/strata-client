...
export default function Credentials({ vaultConfigured }: { vaultConfigured: boolean }) {
  ...
  const load = useCallback(async () => {
    try {
      const [profs, conns] = await Promise.all([getCredentialProfiles(), getMyConnections()]);
      setProfiles(profs);
          ...
          <div className="flex items-center gap-3 text-warning">
            ...
            <div>
              ...
              <p className="text-txt-secondary text-sm mt-1">
                Credential profiles require HashiCorp Vault for secure encryption. Ask an
                administrator to configure Vault in Admin Settings.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="animate-fade-up" style={{ padding: "2rem", maxWidth: 900, margin: "0 auto" }}>
      ...
      {(msg || error) && (
        <div
          className={`rounded-sm mb-4 px-4 py-2 text-[0.8125rem] ${error ? "bg-danger-dim text-danger" : "bg-success-dim text-success"}`}
        >
          {error || msg}
        </div>
      )}
      ...
      {tab === "my-checkouts" && (
        <div>
          <button className="btn btn-sm mb-4" onClick={load}>
            Refresh
          </button>
                ...
                .filter((c) => {
                  ...
                  if (c.status === "Expired" || isCheckoutStale(c) || isCheckoutExpired(c)) {
                    const hasNewer = allCheckouts.some(
                      (other) =>
                        other.managed_ad_dn === c.managed_ad_dn &&
                        other.id !== c.id &&
      ...
      {tab === "profiles" && (
        <>
          ...
          {activeCheckouts.filter((c) => isCheckoutLive(c)).length > 0 && (
            <div className="mb-6">
              <h2 className="text-sm font-semibold text-txt-secondary mb-3 uppercase tracking-wider">
                Active Checkouts
              </h2>
          ...
          {profiles.length === 0 && !editing ? (
            <div className="card text-center py-12">
              ...
              <p className="text-txt-secondary text-sm">
                No credential profiles yet. Create one to securely store your remote server
                credentials.
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {profiles
                  ...
                  if (p.label.startsWith("[managed]")) {
                    const isLinked = profiles.some(
                      (other) =>
                        other.checkout_id &&
                        other.id !== p.id &&
                        allCheckouts.find((c) => c.id === other.checkout_id)?.managed_ad_dn ===
                          p.label.replace("[managed] ", "")
                    );
                    return !isLinked;
                  }
                  return true;
                ...
                .map((profile) => {
                  ...
                  const linkedCo = profile.checkout_id
                    ? allCheckouts.find((c) => c.id === profile.checkout_id)
                    : null;
                    ...
                    <div
                      ...
                      style={
                        isEffectivelyExpired
                          ? { borderColor: "var(--color-danger)", borderWidth: 1 }
                          : undefined
                      }
                    >
                      {/* Profile header */}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-txt-tertiary text-xs ml-3">
                              {profileMappings.length} connection
                              {profileMappings.length !== 1 ? "s" : ""}
                            </span>
                            {linkedActive && linkedCo?.expires_at ? (
                              <span
                                className={`ml-3 text-xs font-semibold ${
                                  new Date(linkedCo.expires_at).getTime() - Date.now() < 300000
                                    ? "text-danger"
                                    : new Date(linkedCo.expires_at).getTime() - Date.now() < 900000
                                      ? "text-warning"
                                      : "text-success"
                                }`}
                              >
                        ...
                        <div className="px-5 py-4" style={{ background: "var(--color-surface)" }}>
                          {profileMappings.length > 0 ? (
                            ...
                            <p className="text-txt-tertiary text-sm mb-3">
                              No connections mapped. Add a connection below so these credentials are
                              used automatically.
                            </p>
                          )}

                          {/* Linked checkout badge / link controls */}
                          {!profile.label.startsWith("[managed]") &&
                            (activeCheckouts.filter((c) => isCheckoutLive(c)).length > 0 ||
                              profile.checkout_id) && (
                              <div
                                className="mt-3 pt-3"
                                ...
                                {profile.checkout_id ? (
                                  ...
                                  <div>
                                    ...
                                    <button
                                      ...
                                      onClick={() => setLinkingProfileId(null)}
                                    >
                                      Cancel
                                    </button>
                                  </div>
                          ...
                          {isAddingMapping ? (
                            ...
                            >
                                      ...
                                      >
                                        ...
                                        <button
                                          ...
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setMappingConnectionIds(
                                              mappingConnectionIds.filter((id) => id !== cid)
                                            );
                                          }}
                                        >
                                          ×
                                ...
                                {mappingDropdownOpen &&
                                  createPortal(
                                    ...
                                    >
                                      ...
                                      <ul
                                        className="max-h-52 overflow-y-auto list-none m-0 p-1"
                                        role="listbox"
                                      >
                                        {filteredAvailable.length === 0 && (
                                          <li className="px-3 py-2 text-sm text-txt-tertiary">
                                        ...
                                        {filteredAvailable.map((c) => {
                                          ...
                                          return (
                                            ...
                                            >
                                              <span
                                                className={`w-4 h-4 rounded-sm border flex items-center justify-center shrink-0 ${isSelected ? "bg-accent border-accent" : "border-txt-tertiary"}`}
                                                style={
                                                  isSelected
                                                    ? {
                              ...
                              <div className="flex items-center gap-3 mt-3">
                                <button
                                  className="btn-primary !py-[0.55rem]"
                                  onClick={handleAddMapping}
                                  disabled={mappingConnectionIds.length === 0}
                                >
                                  Map{" "}
                                  {mappingConnectionIds.length > 0
                                    ? `(${mappingConnectionIds.length})`
                                    : ""}
                                </button>
                                <button
                                  className="btn !py-[0.55rem]"
                                  onClick={() => {
                                    setMappingProfileId(null);
                                    setMappingConnectionIds([]);
                                    setMappingSearch("");
                            ...
                            <button
                              ...
                              onClick={() => {
                                setMappingProfileId(profile.id);
                                setMappingConnectionIds([]);
                                setMappingSearch("");
                              }}
                            >
            ...
            >
              ...
              <p className="text-txt-secondary text-sm mb-6">
                ...
                <span className="text-txt-primary font-semibold">
                  scrambled in Active Directory
                </span>{" "}
                and the checkout will be ended. The account will become available for checkout
                again.
              </p>
              ...
              <div className="flex gap-3">
                ...
                <button className="btn flex-1" onClick={() => setCheckinId(null)}>
                  Cancel
                </button>
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              <p className="text-txt-secondary text-sm mb-6">
                ...
                <span className="text-txt-primary font-semibold">
                  {profiles.find((p) => p.id === deletingId)?.label}
                </span>
                ? This will unmap it from{" "}
                <span className="text-txt-primary font-semibold">
                  {mappings[deletingId]?.length || 0}
                </span>{" "}
              ...
              <div className="flex gap-3">
                ...
                <button className="btn flex-1" onClick={() => setDeletingId(null)}>
                  Cancel
                </button>
              </div>
            </div>
