    ...
    <div className="flex items-center justify-center min-h-screen">
      <div className="w-full max-w-[400px] animate-fade-up">
        ...
        <div className="card">
          ...
          {status?.sso_enabled && status.sso_providers && status.sso_providers.length > 0 && (
            <div className="space-y-3">
              {status.sso_providers.map((provider) => (
                <a
                  key={provider.id}
                  href={`/api/auth/sso/login?provider=${provider.id}`}
              ...
              <p className="text-[0.7rem] text-center text-txt-tertiary">
                Secure enterprise login via OIDC
              </p>
            </div>
          )}
