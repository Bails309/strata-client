...
const mockTunnel = {
  get oninstruction() {
    return mockTunnelOninstruction;
  },
  set oninstruction(fn: any) {
    mockTunnelOninstruction = fn;
  },
  get onerror() {
  ...
  get onstatechange() {
    return mockTunnelOnstatechange;
  },
  set onstatechange(fn: any) {
    mockTunnelOnstatechange = fn;
  },
  sendMessage: vi.fn(),
};
...
describe("HistoricalPlayer", () => {
  ...
  it("toggles play/pause on button click", () => {
    ...
    const pauseBtn = screen
      .getAllByRole("button")
      .find((b) => b.querySelector('svg path[d*="M6 4h4v16H6"]'));
  ...
  it("resumes playback when pause button clicked again", () => {
    ...
    const playBtn = screen
      .getAllByRole("button")
      .find((b) => b.querySelector('svg path[d*="M8 5v14l11-7z"]'));
  ...
  it("does not skip below 0", () => {
    ...
    const backButtons = screen.getAllByTitle(/Skip back/);
    act(() => {
      fireEvent.click(backButtons[3]);
    }); // Skip back 5m (would go below 0)
  ...
  it("uses custom streamUrlBuilder when provided", () => {
    ...
    render(
      <HistoricalPlayer
        ...
        onClose={onClose}
        streamUrlBuilder={customBuilder}
      />
    );
