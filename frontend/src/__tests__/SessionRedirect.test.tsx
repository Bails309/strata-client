...
describe("SessionClient – auto-redirect on session end", () => {
  ...
  beforeEach(() => {
    ...
    mockGetSession.mockImplementation((cid: string) =>
      state.sessions.find((s: any) => s.connectionId === cid)
    );
  ...
  it("redirects to remaining session on server disconnect", async () => {
    ...
    await rtlAct(async () => {
      sessionA.tunnel.oninstruction("disconnect", []);
    });
    await rtlAct(async () => {
      sessionA.tunnel.onstatechange(2);
    }); // CLOSED
