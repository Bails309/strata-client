    ...
    for entry in &entries {
        if let Some((ip, port_str)) = entry.rsplit_once(':') {
            ...
            let port: u16 = port_str
                .parse()
                .map_err(|_| AppError::Validation(format!("Invalid port: {port_str}")))?;
            if port == 0 {
                return Err(AppError::Validation("Port cannot be zero".into()));
            }
        } else if !is_valid_ipv4(entry) {
            return Err(AppError::Validation(format!(
                "Invalid DNS server IP: {entry}"
            )));
        }
    }
    Ok(entries)
...
fn write_resolv_conf(servers: &[String], search_domains: &[String]) -> Result<(), AppError> {
    ...
    // Preserve Docker's embedded DNS as a fallback so connections that
    // resolve via public DNS (or Docker service discovery) keep working.
    content.push_str("nameserver 127.0.0.11\n");
    std::fs::write(&path, content).map_err(|e| {
        AppError::Config(format!(
            "Failed to write DNS config to {}: {e}",
            path.display()
        ))
    })?;
    tracing::info!(
        "Wrote DNS resolv.conf with {} nameserver(s) to {}",
        servers.len(),
        path.display()
    );
    Ok(())
...
pub async fn update_dns(
    ...
    let search_domains = if body.dns_enabled && !body.dns_search_domains.trim().is_empty() {
        parse_and_validate_search_domains(&body.dns_search_domains)?
    } else {
        vec![]
    };
    ...
    if body.dns_enabled && !servers.is_empty() {
        write_resolv_conf(&servers, &search_domains)?;
    } else {
    ...
    audit::log(
        ...
        Some(user.id),
        "dns.updated",
        &json!({ "enabled": body.dns_enabled, "servers": servers, "search_domains": search_domains }),
    )
    .await?;
...
pub async fn test_pm_target_filter(
    ...
    .await
    {
        Ok(users) => {
            let count = users.len();
            let sample: Vec<serde_json::Value> = users
                .iter()
                .take(25)
                .map(|u| {
                    json!({
...
use crate::services::checkouts::{ApprovalRole, CheckoutRequest, UserAccountMapping};
