...
describe("Credentials", () => {
  ...
  it("shows validation error when required fields are empty", async () => {
    ...
    await user.click(screen.getByText("Create Profile"));
    expect(
      await screen.findByText("All fields are required for a new profile")
    ).toBeInTheDocument();
  ...
  it("displays connection count per profile", async () => {
    ...
    vi.mocked(getProfileMappings).mockImplementation(async (id: string) => {
      if (id === "p1")
        return [{ connection_id: "conn1", connection_name: "Server Alpha", protocol: "rdp" }];
      return [];
    });
    renderCredentials();
  ...
  it("updates a credential profile", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText("e.g. Domain Admin, SSH Dev Server"),
      "Updated Profile"
    );
    await user.click(screen.getByText("Update"));
    expect(updateCredentialProfile).toHaveBeenCalledWith(
      "p1",
      expect.objectContaining({ label: "Updated Profile" })
    );
  });
  ...
  it("shows all-accounts-active message on request tab when all have checkouts", async () => {
    ...
    await user.click(screen.getByText("Request Checkout"));
    expect(
      await screen.findByText(/All managed accounts already have active checkouts/)
    ).toBeInTheDocument();
  });
  ...
  it("shows scheduled-start datetime input when Schedule checkbox toggled", async () => {
    ...
    const scheduleCb = screen.getByRole("checkbox", {
      name: /Schedule release for a future time/i,
    });
