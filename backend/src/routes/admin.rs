...
/// Parse and validate the comma-separated list of DNS server addresses.
/// Accepts `ip` or `ip:port` forms.
fn parse_and_validate_dns_servers(raw: &str) -> Result<Vec<String>, AppError> {
    let entries: Vec<String> = raw.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
    for entry in &entries {
        if let Some((ip, port_str)) = entry.rsplit_once(':') {
            if !is_valid_ipv4(ip) {
                return Err(AppError::Validation(format!("Invalid DNS server IP: {ip}")));
            }
            let port: u16 = port_str.parse().map_err(|_| AppError::Validation(format!("Invalid port: {port_str}")))?;
            if port == 0 {
                return Err(AppError::Validation("Port cannot be zero".into()));
            }
        } else if !is_valid_ipv4(entry) {
            return Err(AppError::Validation(format!("Invalid DNS server IP: {entry}")));
        }
    }
    Ok(entries)
}

/// Write a resolv.conf file to /app/config/ so guacd containers can mount it.
fn write_resolv_conf(servers: &[String]) -> Result<(), AppError> {
    let config_dir = std::path::Path::new("/app/config");
    if !config_dir.exists() {
        return Ok(()); // No config volume mounted — skip silently
    }
    let path = config_dir.join("resolv.conf");
    let mut content = String::from("# Auto-generated by Strata — do not edit manually\n");
    for server in servers {
        // resolv.conf only accepts IP addresses, not ip:port
        let ip = if let Some((ip, _port)) = server.rsplit_once(':') {
            ip
        } else {
            server.as_str()
        };
    ...
    std::fs::write(&path, content).map_err(|e| {
        AppError::Config(format!("Failed to write DNS config to {}: {e}", path.display()))
    })?;
    tracing::info!("Wrote DNS resolv.conf with {} nameserver(s) to {}", servers.len(), path.display());
    Ok(())
}
...
pub async fn update_dns(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
    Json(body): Json<DnsUpdateRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;

    let servers = if body.dns_enabled && !body.dns_servers.trim().is_empty() {
        parse_and_validate_dns_servers(&body.dns_servers)?
    } else {
        vec![]
    };
    ...
    if body.dns_enabled && !servers.is_empty() {
        ...
        // Remove the file if DNS is disabled so guacd falls back to defaults
        let path = std::path::Path::new("/app/config/resolv.conf");
        if path.exists() {
    ...
    audit::log(
        ...
        Some(user.id),
        "dns.updated",
        &json!({ "enabled": body.dns_enabled, "servers": servers }),
    )
    .await?;
    ...
    Ok(Json(json!({
        ...
        "message": "DNS settings saved. Restart guacd services to apply: docker-compose restart guacd"
    })))
}
...
pub async fn list_ad_sync_configs(
    ...
    for r in &mut rows {
        ...
        if let Some(ref pw) = r.pm_bind_password {
            if !pw.is_empty() {
                r.pm_bind_password = Some(DOT_MASK.into());
            }
        }
    }
    Ok(Json(rows))
...
pub struct CreateAdSyncConfigRequest {
    ...
    pub pm_target_filter: Option<String>,
...
pub async fn create_ad_sync_config(
    ...
    // Encrypt pm_bind_password if provided
    let stored_pm_password = if let Some(ref pw) = body.pm_bind_password {
        if !pw.is_empty() && pw != DOT_MASK && pw != STAR_MASK {
            if pw.starts_with("vault:") {
                Some(pw.clone())
            } else {
                let vault_cfg = {
                    let s = state.read().await;
                ...
                if let Some(ref vc) = vault_cfg {
                    Some(crate::services::vault::seal_setting(vc, pw).await?)
                } else {
                    Some(pw.clone())
                }
            }
...
pub async fn test_ad_sync_connection(
    ...
    let config = AdSyncConfig {
        ...
        pm_target_filter: String::new(),
        pm_pwd_min_length: 16,
...
// ── Test PM target filter (preview accounts) ───────────────────────────

pub async fn test_pm_target_filter(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
    Json(body): Json<CreateAdSyncConfigRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    ...
    // Resolve bind password (handle mask values)
    let mut bind_password = body.bind_password.clone().unwrap_or_default();
    if bind_password == DOT_MASK || bind_password == STAR_MASK {
        if let Some(id) = body.id {
            let existing: Option<String> =
                sqlx::query_scalar("SELECT bind_password FROM ad_sync_configs WHERE id = $1")
                    ...
                    .fetch_optional(&db.pool)
                    .await?;
            if let Some(pw) = existing { bind_password = pw; }
        }
    }

    // Resolve PM bind password if separate creds are used
        ...
        if let Some(id) = body.id {
            let existing: Option<String> =
                sqlx::query_scalar("SELECT pm_bind_password FROM ad_sync_configs WHERE id = $1")
                    ...
                    .fetch_optional(&db.pool)
                    .await?;
            if let Some(pw) = existing { pm_bind_pw = pw; }
        }
    }

    // Unseal vault-prefixed passwords
    let vault_cfg = {
        let s = state.read().await;
        s.config.as_ref().and_then(|c| c.vault.clone())
    };
    if let Some(ref vc) = vault_cfg {
        ...
        if pm_bind_pw.starts_with("vault:") {
            pm_bind_pw = crate::services::vault::unseal_setting(vc, &pm_bind_pw).await?;
        }
    }

    // Use PM-specific credentials if provided, otherwise fall back to main bind creds
    let effective_bind_dn = body.pm_bind_user.clone()
        .filter(|s| !s.is_empty())
    ...
    let filter = body.pm_target_filter.clone()
    ...
    .await
    {
        Ok(users) => {
            let count = users.len();
            let sample: Vec<serde_json::Value> = users.iter().take(25).map(|u| {
                json!({
                    "dn": u.dn,
                    "name": u.name,
            ...
            Ok(Json(json!({
                "status": "success",
                "message": format!("Filter matched {} account(s)", count),
                "count": count,
                "sample": sample,
            })))
        }
        Err(e) => {
            let raw = format!("{e}");
            let hint = parse_ldap_error_hint(&raw);
            Ok(Json(json!({
                "status": "error",
                "message": raw,
                "hint": hint,
...
/// Parse common AD/LDAP error codes and return a human-readable hint.
pub fn parse_ldap_error_hint(msg: &str) -> Option<String> {
    // AD embeds sub-status codes as "data <hex>" inside LDAP result messages
    // Reference: https://ldapwiki.com/wiki/Common%20Active%20Directory%20Bind%20Errors
    let lower = msg.to_lowercase();

    // rc=49 sub-status codes (invalidCredentials family)
    if lower.contains("data 525") {
        return Some("User not found — the bind DN does not match any account in Active Directory. Verify the full distinguished name is correct.".into());
    }
    if lower.contains("data 52e") {
        return Some("Invalid credentials — the password is incorrect for this account. If the password was recently changed, update it here and try again.".into());
    }
    if lower.contains("data 530") {
        return Some("Logon hours restriction — this account is not permitted to log on at this time. Check the account's 'Logon Hours' in Active Directory.".into());
    }
    if lower.contains("data 531") {
        return Some("Workstation restriction — this account is only allowed to log on from specific machines. Check 'Log On To...' in the account properties and add this server, or remove the restriction.".into());
    }
    if lower.contains("data 532") {
        return Some("Password expired — the account's password has expired. Reset the password in Active Directory and update the credentials here.".into());
    }
    if lower.contains("data 533") {
        return Some("Account disabled — this account is disabled in Active Directory. Enable the account or use a different service account.".into());
    }
    if lower.contains("data 52f") {
        return Some("Account restriction — a policy is preventing this account from authenticating. Common causes: 'User must change password at next logon' is enabled, logon workstation restrictions, logon hours restrictions, or a fine-grained password policy blocking LDAP simple binds. Check the account in ADUC → Properties → Account tab.".into());
    }
    if lower.contains("data 701") {
        return Some("Account expired — this account has passed its expiration date. Extend the expiration in Active Directory or use a different service account.".into());
    }
    if lower.contains("data 773") {
        return Some("Password must be changed — 'User must change password at next logon' is set on this account. Log in interactively to change the password first, or clear that flag in ADUC.".into());
    }
    if lower.contains("data 775") {
        return Some("Account locked out — too many failed authentication attempts have locked this account. Unlock it in Active Directory (ADUC → right-click → Unlock Account) and verify the correct password.".into());
    }

    // Generic rc=49 without a sub-code
    if lower.contains("rc=49") || lower.contains("invalidcredentials") {
        return Some("Invalid credentials — the bind DN or password is incorrect. Verify both values are correct. If using a DN, ensure it is the full distinguished name (e.g. CN=svc-account,OU=Service Accounts,DC=contoso,DC=com).".into());
    }

    // rc=1 (operations error) — often means anonymous bind not allowed
    if lower.contains("rc=1") && lower.contains("operations error") {
        return Some("Operations error — the server rejected the request. This often means the bind credentials were empty or anonymous binds are disabled. Verify the bind DN and password are set correctly.".into());
    }

    // rc=32 (no such object) — bad search base
    if lower.contains("rc=32") || lower.contains("no such object") {
        return Some("No such object — the search base DN does not exist. Verify your search base matches an actual OU or container in Active Directory.".into());
    }

    // TLS / connection errors
    if lower.contains("tls") || lower.contains("certificate") || lower.contains("ssl") {
        return Some("TLS/SSL error — the LDAP connection could not establish a secure channel. If using a self-signed or internal CA certificate, paste the CA cert in PEM format or enable 'Skip TLS Verification' for testing.".into());
    }

    // Connection refused / timeout
    if lower.contains("connection refused") || lower.contains("connect error") {
        return Some("Connection refused — cannot reach the LDAP server. Verify the LDAP URL, port, and that the server is reachable from this network.".into());
    }
    if lower.contains("timeout") || lower.contains("timed out") {
        return Some("Connection timed out — the LDAP server did not respond. Check network connectivity, firewall rules, and that the LDAP URL and port are correct.".into());
    }
...
// ════════════════════════════════════════════════════════════════════════
// Password Management — Approval Roles, Account Mappings, Checkout Admin
// ════════════════════════════════════════════════════════════════════════

use crate::services::checkouts::{
    ApprovalRole, CheckoutRequest, UserAccountMapping,
};
...
pub async fn list_approval_roles(
    ...
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    let rows: Vec<ApprovalRole> =
        sqlx::query_as("SELECT * FROM approval_roles ORDER BY name")
            .fetch_all(&db.pool)
            .await?;
    Ok(Json(rows))
}
...
pub async fn create_approval_role(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
    Json(body): Json<CreateApprovalRoleRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    ...
    audit::log(
        &db.pool,
        Some(user.id),
        "approval_role.created",
        &json!({ "role_id": id, "name": name }),
    )
    .await?;
    Ok(Json(json!({ "id": id, "status": "created" })))
}
...
pub async fn update_approval_role(
    ...
    Json(body): Json<UpdateApprovalRoleRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    ...
    if let Some(ref desc) = body.description {
        sqlx::query(
            "UPDATE approval_roles SET description = $1, updated_at = now() WHERE id = $2",
        )
        .bind(desc)
        .bind(role_id)
        .execute(&db.pool)
...
#[derive(Deserialize)]
pub struct SetRoleAssignmentsRequest {
...
pub async fn set_role_assignments(
    ...
    Extension(user): Extension<AuthUser>,
    Path(role_id): Path<Uuid>,
    Json(body): Json<SetRoleAssignmentsRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    let mut tx = db.pool.begin().await?;
...
pub async fn set_role_accounts(
    ...
    Extension(user): Extension<AuthUser>,
    Path(role_id): Path<Uuid>,
    Json(body): Json<SetRoleAccountsRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    let mut tx = db.pool.begin().await?;
...
// ── Unmapped account discovery ─────────────────────────────────────────

pub async fn list_unmapped_accounts(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
    Path(config_id): Path<Uuid>,
) -> Result<Json<Vec<serde_json::Value>>, AppError> {
    ...
    if !config.pm_enabled {
        return Err(AppError::Validation(
            "Password management is not enabled for this config".into(),
        ));
    }

    // Decrypt bind passwords
    let vault_cfg = {
        let s = state.read().await;
        s.config.as_ref().and_then(|c| c.vault.clone())
    };
    ...
    let bind_pw = if let Some(ref vc) = vault_cfg {
        crate::services::vault::unseal_setting(vc, raw_pw).await?
    } else {
        raw_pw.to_string()
    };
    ...
    let existing_dns: Vec<String> =
        sqlx::query_scalar("SELECT managed_ad_dn FROM user_account_mappings WHERE ad_sync_config_id = $1")
            .bind(config_id)
            .fetch_all(&db.pool)
            .await?;
...
async fn ad_sync_discover_users(
    ldap_url: &str,
    bind_dn: &str,
    bind_password: &str,
    search_bases: &[String],
...
) -> Result<Vec<DiscoveredUser>, AppError> {
    ...
    let bases = if search_bases.is_empty() {
        vec![String::new()]
    } else {
        search_bases.to_vec()
    };
    ...
    for base in &bases {
        let (results, _) = tokio::time::timeout(
            ...
            ldap.search(
                ...
                Scope::Subtree,
                filter,
                vec!["cn", "sAMAccountName", "distinguishedName", "description"],
            ),
        )
        .await
        .map_err(|_| AppError::Internal("LDAP search timed out".into()))?
        .map_err(|e| AppError::Internal(format!("LDAP search: {e}")))?
        .success()
...
// ── User account mappings (assign managed AD accounts to Strata users) ─
...
pub async fn create_account_mapping(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
    Json(body): Json<CreateAccountMappingRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    let id: Uuid = sqlx::query_scalar(
    ...
    audit::log(
        &db.pool,
        Some(user.id),
        "account_mapping.created",
        &json!({ "mapping_id": id, "target_user": body.user_id, "dn": body.managed_ad_dn }),
    )
    .await?;
    Ok(Json(json!({ "id": id, "status": "created" })))
}
...
#[derive(Deserialize)]
pub struct TestRotationRequest {
    pub config_id: Uuid,
}
...
pub async fn test_rotation(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
    Json(body): Json<TestRotationRequest>,
) -> Result<Json<serde_json::Value>, AppError> {
    crate::services::middleware::check_system_permission(&user)?;
    let db = require_running(&state).await?;
    let vault_cfg = {
        ...
        s.config
            ...
            .ok_or(AppError::Validation("Vault not configured".into()))?
    };

    match crate::services::checkouts::auto_rotate_service_account(
        &db.pool,
        &vault_cfg,
        body.config_id,
    )
    .await
    {
        Ok(()) => Ok(Json(json!({
            "success": true,
            "message": "Service account password rotation successful. New password verified and sealed."
        }))),
        Err(e) => Ok(Json(json!({
            "success": false,
...
// ── Admin: list all checkout requests ──────────────────────────────────

pub async fn list_checkout_requests(
    State(state): State<SharedState>,
    Extension(user): Extension<AuthUser>,
