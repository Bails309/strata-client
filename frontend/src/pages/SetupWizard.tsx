    ...
    <div className="flex items-center justify-center min-h-screen">
      <div className="card w-full max-w-[520px]">
        ...
        <p className="text-txt-secondary mb-6">
          The database is configured automatically via environment variables.
          Configure Vault for credential encryption below.
        </p>

        {error && (
          <div className="rounded-md mb-4 px-4 py-2 bg-danger-dim text-danger">
            {error}
          </div>
        )}
        ...
        <div className="form-group">
          ...
          <div className="grid gap-2">
            {([
              { value: 'local' as VaultMode, label: 'Bundled Vault', desc: 'Auto-configured container — no setup required' },
              { value: 'external' as VaultMode, label: 'External Vault', desc: 'Connect to your own HashiCorp Vault instance' },
              { value: 'skip' as VaultMode, label: 'Skip for Now', desc: 'Use local encryption only — configure Vault later' },
            ]).map((opt) => (
              <label
                key={opt.value}
