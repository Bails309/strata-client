...
describe('FileBrowser', () => {
  ...
  it('downloads a file on double click', async () => {
    ...
    let callCount = 0;
    fs.object.requestInputStream = vi.fn((_path: string, cb: (stream: any, mimetype: string) => void) => {
      callCount++;
      if (callCount === 1) {
        // Root directory load
        cb({}, 'application/vnd.glyptodon.guacamole.stream-index+json');
  ...
  it('triggers upload when files are selected', async () => {
    ...
    vi.mocked((await import('guacamole-common-js')).default.BlobWriter).mockImplementation(function() {
  ...
  it('uses application/octet-stream for files without type', async () => {
    ...
    expect(fs.object.createOutputStream).toHaveBeenCalledWith('application/octet-stream', '/unknown');
  });
});
