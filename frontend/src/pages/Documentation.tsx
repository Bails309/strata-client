...
import { getRoadmapStatuses, setRoadmapStatus, type MeResponse, type RoadmapStatus as ApiRoadmapStatus } from '../api';
...
function RoadmapContent({ user }: { user?: MeResponse | null }) {
  ...
  useEffect(() => {
    let cancelled = false;
    getRoadmapStatuses()
      .then(res => { if (!cancelled) setOverrides(res.statuses || {}); })
      .catch(err => { if (!cancelled) setLoadError(err?.message || 'Failed to load roadmap statuses'); });
    return () => { cancelled = true; };
  }, []);

  const effectiveStatus = (item: RoadmapItem): RoadmapStatus =>
    overrides[item.id] ?? item.status;

  const totals = useMemo(() => {
    const t: Record<RoadmapStatus, number> = {
      Proposed: 0, Researching: 0, 'In Progress': 0, Shipped: 0,
    };
  ...
  const handleChange = async (itemId: string, next: RoadmapStatus) => {
    ...
    try {
      ...
      setOverrides(o => {
        const copy = { ...o };
        if (prev === undefined) delete copy[itemId]; else copy[itemId] = prev;
        return copy;
      });
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-txt-tertiary max-w-2xl">
          Proposed feature work beyond the current shipped releases. Items here represent direction and intent — not committed delivery dates. Shipped features move to the <strong className="text-txt-secondary font-semibold">What's New</strong> tab.
        </p>
        {loadError && (
        ...
        {saveError && (
          <p className="mt-2 text-xs" style={{ color: '#f87171' }}>{saveError}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {(Object.keys(totals) as RoadmapStatus[]).map(s => {
          ...
          return (
            ...
            >
              <div className="text-xs uppercase tracking-wider text-txt-tertiary font-semibold mb-1">{s}</div>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold" style={{ color: fg }}>{totals[s]}</span>
                <span className="w-2 h-2 rounded-full" style={{ background: bg, boxShadow: `0 0 0 2px ${fg}33` }} />
              </div>
            </div>
          );
        })}
      ...
      {ROADMAP.map(theme => (
        <section key={theme.id} className="space-y-4">
          <div className="flex items-center gap-3 pb-2" style={{ borderBottom: '1px solid var(--color-border)' }}>
            ...
            <div>
              <h2 className="text-base font-semibold text-txt-primary leading-tight">{theme.title}</h2>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-sm font-semibold text-txt-primary leading-snug">{item.title}</h3>
                    {canEdit ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.areas.map(a => (
                      <span
                        ...
                        style={{ background: 'var(--color-surface-tertiary)', color: 'var(--color-txt-secondary)' }}
                      >
                        {a}
                      </span>
                    ))}
                  </div>
                  <p className="text-[0.8125rem] text-txt-secondary leading-relaxed">{item.description}</p>
                  {item.bullets && (
                    <ul className="text-[0.8125rem] text-txt-secondary leading-relaxed space-y-1 pl-4" style={{ listStyle: 'disc' }}>
      ...
      <div
        ...
        style={{ background: 'var(--color-surface-secondary)', border: '1px dashed var(--color-border)' }}
      >
        Have a feature suggestion? Raise an issue in the project tracker and tag it{' '}
        <code className="px-1 py-0.5 rounded text-txt-secondary" style={{ background: 'var(--color-surface-tertiary)' }}>roadmap</code>.
      </div>
    </div>
  );
}
