...
export const RELEASE_CARDS: ReleaseCard[] = [
  {
    version: "1.12.11",
    subtitle:
      "Patch release — Safeguard auto-request pending state is finally visible on Credentials → Request Checkout. A customer reported that connecting directly to a connection whose credential profile is mapped to a Safeguard account requiring approver action (\"auto-request on direct-connect\") never updated the credentials page to say Pending Approval, so every subsequent connect attempt looked like a fresh JIT checkout to the user. The backend's reuse_pending preflight kept us from creating actual duplicate access requests on the appliance, but the SPA gave the user no way to see that anything was pending, and the customer reasonably read that as 'the auto-request feature keeps re-requesting'. Root cause: routes::tunnel::ws_tunnel's JitOutcome::PendingApproval branch stored the request_id only in its own stack frame — nothing was persisted anywhere the Credentials → Request Checkout tab could read back, so the yellow Awaiting approval badge, the manual Refresh button, and the background poll useEffect all stayed silent. v1.12.11 introduces a new safeguard_pending_requests table that every PendingApproval observation writes into (auto-request AND manual bulk-checkout), clears on the two terminal paths (successful release, check-in), and exposes via a new GET /api/user/safeguard/pending endpoint. The bulk-checkout card now hydrates from /pending on mount and merges the persistent rows into its results state so the badge, the Refresh button, and the poll clock all appear the very next time the user opens the Credentials page. Additive-only merge so an in-session ok row is never clobbered by a stale pending record. Two new regression tests pin the mount-hydration path and the anti-clobber rule (21 → 23 passing cases in SafeguardBulkCheckoutCard.test.tsx). New migration 079_safeguard_pending_requests.sql, one new endpoint, one new service module, no new env vars, no new Cargo or npm dependencies.",
    sections: [
      {
        title:
          "Symptom — auto-request against an approval-required Safeguard account kept 're-requesting'",
        description:
          "Reproduce against v1.12.10 on a deployment with a Safeguard-mapped connection whose credential_profile references an account that requires approver action (require_approval=true on the appliance side). Sign in as a user who has never manually requested that profile from the Credentials → Request Checkout tab. From the Dashboard, click Connect on the connection. The tunnel WebSocket upgrade fails with a generic browser 'Connection failed' toast. Navigate to Credentials → Request Checkout — the safeguard-mapped profile shows no pending state, no yellow Awaiting approval badge, no Refresh button, no request-id, and no background poll indicator. Click Connect again on the Dashboard: same generic failure. The customer's read: 'the auto-request feature keeps re-requesting'. Actual backend behaviour: services::safeguard::jit_checkout's list_my_active_requests_for preflight sees the still-open request on the appliance and reuse_pending short-circuits — the appliance is NOT accumulating duplicate access requests. But the SPA's Credentials tab has no way to know the appliance already has an open request for this user, so the user has no place to wait for the approver decision. The only workaround was to press Request Checkout manually in the bulk-checkout card, which persisted the request-id in local useState so the badge and the Refresh button appeared — but that ceremony was invisible to the user experiencing the auto-request path.",
      },
      {
        title: "Root cause — routes::tunnel::ws_tunnel's PendingApproval branch persisted nothing",
        description:
          "backend/src/routes/tunnel.rs ws_tunnel handles direct connects. When the connection's credential profile is kind='safeguard' and the appliance has password caching enabled but no valid cache row exists, ws_tunnel calls services::safeguard::jit_checkout — which returns JitOutcome::Released { password, request_id, ... } on the happy path (approver already approved OR the account has no approval role) or JitOutcome::PendingApproval { request_id, ... } when the appliance rejects the checkout because an approver has not yet acted. The pre-v1.12.11 PendingApproval arm captured request_id only for the AppError::Validation formatted message ('safeguard.approval_required:{request_id}') and returned. The message bubbled up to a 400 response body — but the browser hides HTTP 400 bodies from failed WebSocket handshakes, so the SPA saw only the generic close event. Meanwhile the manual bulk-checkout path (routes::user::bulk_safeguard_checkout) had the same problem: its own JitOutcome::PendingApproval arm surfaced the request_id via the JSON response body, which the SafeguardBulkCheckoutCard cached in useState — but that state lived only in the mounted card and evaporated on page reload or cross-browser handoff, and it never learned about the auto-request path's pending requests at all.",
      },
      {
        title:
          "Fix — persist every PendingApproval observation server-side, expose via /user/safeguard/pending, hydrate on card mount",
        description:
          "New migration 079_safeguard_pending_requests.sql creates safeguard_pending_requests(user_id UUID FK users, profile_id UUID FK credential_profiles, request_id TEXT, account_id TEXT, asset TEXT, created_at TIMESTAMPTZ DEFAULT now(), PRIMARY KEY(user_id, profile_id)) with ON DELETE CASCADE and idx_safeguard_pending_req_user for the list query. New module backend/src/services/safeguard/pending_requests.rs exposes store (idempotent INSERT ... ON CONFLICT DO UPDATE so a reused request-id overwrites cleanly), clear (idempotent DELETE for the two terminal paths), and status_for_user (SELECT for the SPA hydration endpoint). Both the auto-request path (routes::tunnel::ws_tunnel) and the manual bulk-checkout path (routes::user::bulk_safeguard_checkout) now call pending_requests::store on every PendingApproval observation, best-effort so a persist failure never swallows the validation error the caller needs to see. Both terminal paths (release_safeguard_pending → Released and bulk_safeguard_checkin) call pending_requests::clear on the row, and release_safeguard_pending → PendingApproval refreshes the created_at timestamp so the 30-minute poll cap tracks the appliance view. New endpoint GET /api/user/safeguard/pending returns every pending row for the calling user, newest-first. frontend/src/pages/credentials/SafeguardBulkCheckoutCard.tsx's refresh() now fetches /user/safeguard/pending in parallel with listSafeguardCached and merges pending rows into results for any profile_id not already tracked — additive only, so an in-session ok row is never clobbered by a stale pending record — and seeds pollStartedAt from created_at so the 30-minute cap is measured from when the request was originally made. Two new regression tests: 'hydrates a pending row from /user/safeguard/pending on mount' and 'does not overwrite a cached ok row with a stale pending row from /pending'.",
      },
      {
        title: "Acceptance test",
        description:
          "Apply v1.12.11 to a deployment with a Safeguard-mapped connection whose credential_profile references an approval-required account. Migration 079 runs automatically on boot. Sign in as a user who has never manually checked out that profile. From the Dashboard, click Connect on the connection. The tunnel WebSocket upgrade fails (unchanged — the browser hides HTTP 400 bodies from failed WebSocket handshakes) but this time navigate to Credentials → Request Checkout and observe the yellow Awaiting approval badge on the safeguard-mapped profile row, complete with the appliance request-id and the standard 're-check every 15s for up to 30 minutes' hint. Have an approver approve the request in the Safeguard console. Within one poll tick (15s, or press Refresh immediately) the row MUST flip to Checked out with the green Cached · Nh left badge. Return to the Dashboard and click Connect on the connection — the session MUST open normally, loading the password from safeguard_cached_passwords. Verify pending_requests row was cleared: SELECT count(*) FROM safeguard_pending_requests WHERE user_id = <the-user>; MUST return 0. Also verify the negative path: press Check in on the profile row and confirm safeguard_cached_passwords is cleared. Docker deploy: docker compose pull && docker compose up -d --build backend frontend.",
      },
    ],
  },
  {
    version: "1.12.10",
    subtitle:
      "Patch release — the Safeguard bulk-checkout card no longer loses pending (approval-required) rows when a subsequent checkout runs for a different profile. A customer reported that on the Credentials → Request Checkout tab, checking out an ad-hoc privileged profile (approval-required) followed by a plain test profile (no approval) silently wiped the ad-hoc row's yellow Awaiting approval badge, dropped the manual Refresh button from the DOM, and stopped the background poll useEffect from ever noticing the approver's decision — the only recovery was to press Check in all and restart the whole request from scratch. Root cause: both handleCheckout and handleCheckin in frontend/src/pages/credentials/SafeguardBulkCheckoutCard.tsx opened with setResults([]) — wiping the entire per-profile results array — and then re-set results to only the rows for the profiles included in the current operation, so pending rows for unrelated profiles fell out of state entirely. Backend password_cache state was correct throughout; the bug was purely frontend state management. v1.12.10 merges results by profile_id on checkout and prunes by profile_id on check-in, preserving pending rows so the background poll continues to watch them through the standard 30-minute window. No backend changes, no API surface changes, no migrations, no new environment variables, no new Cargo or npm dependencies — strictly frontend.",
    sections: [
      {
        title:
          "Symptom — pending rows disappeared when a second checkout ran for a different profile",
        description:
          "Reproduce against v1.12.9 on Credentials → Request Checkout: sign in to Safeguard; in the Safeguard bulk checkout card, select an ad-hoc privileged profile whose account is under an approval role (the appliance requires an approver to release the password) and click Checkout selected. The row correctly flips to the yellow Awaiting approval badge with a Refresh button and the standard 're-check every 15s for up to 30 minutes' hint text. Then select a second profile — a plain test account that does NOT require approval — and click Checkout selected again. On v1.12.9 the moment the second bulk-checkout dispatches, the ad-hoc row's Awaiting approval badge silently disappears. The test row shows Checked out (or the cached-password green badge after refresh) as expected, but the ad-hoc row has no badge at all and no Refresh button. The background poll loop stops watching for the approver's decision. Even when the approver clicks Approve in the Safeguard console the SPA never notices — the row can only be recovered by pressing Check in all and starting the entire request over from scratch. The same clobber happens via Check in (single row) and Check in all (N).",
      },
      {
        title:
          "Root cause — setResults([]) wiped the entire per-profile results array on every checkout and every check-in",
        description:
          "frontend/src/pages/credentials/SafeguardBulkCheckoutCard.tsx tracks per-profile checkout results in a single results useState array keyed conceptually by profile_id. Both handleCheckout and handleCheckin opened with setResults([]) — wiping the entire array — and then re-set results to only the rows for the profiles included in the current operation. When a subsequent handleCheckout ran for a different subset (e.g. just the test profile), the ad-hoc profile's state === 'pending' row was collateral damage: it was in prev but not in res, so it fell out of state entirely. The background poll useEffect (declared further down in the component) filters results.filter(r => r.state === 'pending' && !!r.request_id) to decide which appliance request-ids to keep re-polling. With the ad-hoc row gone from results, the effect's pending array narrowed to zero, the cleanup ran, and the setInterval was cleared. The manual Refresh button on the ad-hoc row also disappeared because it is conditionally rendered only when result?.state === 'pending' && result.request_id is truthy for that row. The pollStartedAt map still held the ad-hoc profile's start timestamp, but nothing was reading it in a still-pending capacity anymore. The password_cache row on the backend was correctly persisted in the pending state throughout — the bug was purely a frontend state-management regression, no data was lost.",
      },
      {
        title: "Fix — merge results by profile_id, prune only what was actually checked in",
        description:
          "Both mutation paths now preserve unrelated rows. handleCheckout no longer calls setResults([]) on entry; when the appliance response res returns, it does setResults(prev => { const byId = new Map(prev.map(r => [r.profile_id, r])); for (const r of res) byId.set(r.profile_id, r); return Array.from(byId.values()); }). Rows for profiles that were part of the new checkout are replaced (so re-running a stale failed row picks up the new ok/pending state); rows for profiles that weren't part of the new checkout survive untouched. handleCheckin no longer calls setResults([]) on entry; after safeguardCheckin returns, it prunes only the checked-in rows from results (so the now-stale Checked out badge on the row disappears alongside its cached badge from refresh()), never touching pending rows for other profiles. For the Check in all button (profileId === null) the prune targets every currently-cached profile's row while explicitly preserving state === 'pending' rows — those represent requests the appliance is still holding for approver action and must not be dropped from view. A regression test in SafeguardBulkCheckoutCard.test.tsx now pins the exact bug-report click sequence — 18 → 19 passing cases in the component's test file.",
      },
      {
        title: "Acceptance test",
        description:
          "Apply v1.12.10 to a deployment where at least one Safeguard profile targets an account that requires approver action, and at least one other Safeguard profile targets an account that auto-releases. Sign in as any user with both profiles assigned. On Credentials → Request Checkout, sign in to Safeguard. In the Safeguard bulk checkout card enter a justification, select the approval-required profile only and click Checkout selected. Observe the Awaiting approval badge and Refresh button appear on that row. Without checking anything in, select the auto-release profile only and click Checkout selected again. On v1.12.10 the ad-hoc row's yellow badge and Refresh button MUST remain and the auto-release row MUST show Checked out (green badge appears after the sibling refresh() completes). Have the approver approve the ad-hoc request in the Safeguard console. Within one poll tick (15s) the ad-hoc row MUST flip to Checked out and the green Cached · Nh left badge MUST appear alongside it. Press Check in on the auto-release row — confirm the ad-hoc row is unaffected (still cached, badge intact). Press Check in all (N) — confirm every currently-cached row is released, but any row still in the Awaiting approval state (start a fresh one for this step) MUST survive the bulk check-in. No DB migrations, no new env vars, no new Cargo or npm dependencies, no backend code changes — the standard deploy is docker compose pull && docker compose up -d --build frontend.",
      },
    ],
  },
  {
    version: "1.12.9",
    subtitle:
      "Patch release — the Dashboard search box now matches tag names alongside connection name, hostname, and description. Customers organising their fleet with tags (Domain Controllers, Management Servers, Production, etc.) naturally typed the tag into the My Connections search input on /dashboard and saw zero results — the literal word 'domain' does not appear in the name/hostname/description of a server called cicsazt1mgt-p even when that server is tagged Domain Controllers. The dedicated tag-pill chip row below the search continued to work correctly for AND-style narrowing, but the search box was the obvious first thing to try and it silently returned an empty list. v1.12.9 extends the filtered useMemo in frontend/src/pages/Dashboard.tsx with a fourth || clause that also matches the query against the names of every tag assigned to each connection (user-defined and admin-defined tags, both surfaced via the existing allConnTagMap and allTags derivations), and updates the input placeholder from 'Search' to 'Search name, host, description or tag' so the new capability is discoverable. No backend changes, no API surface changes, no migrations, no new environment variables, no new Cargo or npm dependencies — five lines in one frontend file. The in-session Ctrl+K Command Palette already matched against tag names (added in v1.12.4 alongside the open-session prioritisation work) so the two surfaces now agree on what 'search' means.",
    sections: [
      {
        title: "Symptom — typing a tag name into the dashboard search returned zero rows",
        description:
          "A customer running v1.12.8 organised their estate with two admin-defined tags — Domain Controllers (purple chip) and Management Servers (green chip) — and approximately a hundred named connections distributed across them. The operators reported that typing 'domain' into the Search input at the top of the My Connections view returned an empty list, even though the purple Domain Controllers chip was visible directly above the search row and clicking it correctly narrowed the table to four servers. The customer's natural reading of 'the search box on a page that shows tag chips above the table' was that the search box also searches tags; instead it searched only the connection name (cicsazt0mgt-p), hostname (cicsazt0mgt-p.capita-ics.co.uk), and description (none of which contained the literal substring 'domain'). The misunderstanding cost the customer non-trivial time because the dedicated TAGS chip row was easy to miss at a glance on deployments with a small number of tags — the chips render inline next to the Folders / Expand all / Collapse all buttons, not in a labelled filter panel — and because the in-session Ctrl+K Command Palette (which the same operators use day-to-day) has matched tag names against its query since v1.12.4. Operators reasonably expected the dashboard's primary filter input to behave the same way as the global one.",
      },
      {
        title:
          "Root cause — search predicate compared the query only to name, hostname, description",
        description:
          "The filtered useMemo in Dashboard.tsx composed four narrowing predicates against the connections array: showFavorites → search → typeFilter → activeTagFilters. The search predicate compared the lowercased query against three fields per row: c.name.toLowerCase(), c.hostname.toLowerCase(), and (c.description || '').toLowerCase(). There was no tag-name clause, even though both allTags (the merged admin+user tag list) and allConnTagMap (the merged admin+user connection→tag-id map) were already in scope as constants on the same render — they were used by the activeTagFilters clause immediately below the search clause and by the connection-row rendering further down the component. The chip-based tag filter (activeTagFilters) worked correctly because it iterated allConnTagMap[c.id] looking for an id intersection with the toggled chips, but free-text search never crossed into the tag space at all.",
      },
      {
        title: "Fix — fourth || clause matches the query against the names of every assigned tag",
        description:
          "frontend/src/pages/Dashboard.tsx — inside the if (search) branch of the filtered useMemo, build a single Map<tagId, tagName.toLowerCase()> from allTags, then extend the row predicate with a fourth || clause: (allConnTagMap[c.id] || []).some((tid) => (tagNameById.get(tid) || '').includes(q)). The map is built lazily inside the closure so the cost is paid only when the user has actually typed something, and only against the connections that survived the preceding showFavorites narrowing. The useMemo dependency list gains allTags so the result recomputes correctly when the tag inventory changes (rename, create, delete) while the search field is non-empty. The input placeholder was changed from 'Search' to 'Search name, host, description or tag' in the same edit so the new capability is discoverable without having to consult the documentation. The dedicated tag-pill chip filter (activeTagFilters) is unchanged and continues to compose with the search via AND — e.g. typing 'mgt' while the Domain Controllers chip is active returns the intersection.",
      },
      {
        title: "Why this is a patch and not a minor",
        description:
          "Strictly additive behaviour. Every query that returned a match on v1.12.8 still returns the same match on v1.12.9 (the new clause is ||-joined, so existing matches cannot be filtered out by the change). The result set can only grow when the query happens to be a substring of an assigned tag name. Sites that do not use tags at all see zero behavioural change because allConnTagMap[c.id] is [] on every row and the new clause short-circuits to false on the first iteration. Surface area: one file, one useMemo, one placeholder string. No backend code, no API surface, no migration, no env var, no new dependency. Composability is preserved — the chip filter still composes with the search via AND. Performance: the new clause is O(tagsPerConnection) per row per keystroke, with the tagNameById lookup Map built once per filtered recomputation. For a reference 200-connection fleet with an average of two tags per connection the additional work per keystroke is ~400 Map.get calls plus ~400 String.prototype.includes calls — imperceptible against the React render cost of the result list itself.",
      },
      {
        title: "Acceptance test",
        description:
          "Apply v1.12.9 to a deployment that has at least one connection tagged via the connection editor or via the Admin → Tags tab (e.g. tag cicsazt1mgt-p with Domain Controllers). Sign in as any user with access to the tagged connection. Navigate to /dashboard and locate the Search input below the connection tiles. Expected on v1.12.9: the placeholder reads 'Search name, host, description or tag'. Type 'domain' — the row for cicsazt1mgt-p (and every other connection tagged Domain Controllers) is included in the filtered list, even though the literal string 'domain' does not appear in the connection's name, hostname, or description field. Was on v1.12.8: the same query returned zero rows because only the three connection fields were searched. Clear the search input and confirm the full list returns. Click the Domain Controllers chip in the TAGS row and confirm the existing chip-based filter still narrows the table to exactly the same set of rows (the search and chip filters compose via AND, so typing into the search while a chip is active is also valid). As a non-admin user with no tag assignments on any visible connection, type any tag name — the search returns the same name/hostname/description-only result set as on v1.12.8 (the new clause is a no-op when allConnTagMap[c.id] is []). No DB migrations, no new env vars, no new Cargo or npm dependencies, no backend code changes — the standard deploy is docker compose pull && docker compose up -d --build frontend.",
      },
    ],
  },
  {
    version: "1.12.8",
    subtitle:
      "Patch release — UI hotfix: delegated users now see the connection name on session tiles. v1.12.7's tightening of GET /api/admin/connections (now requires can_manage_connections) surfaced an unrelated and previously-silent bug in frontend/src/pages/SessionClient.tsx — the page was using the admin endpoint to translate the URL-supplied connectionId into a human-readable name for the session tile in the sidebar. Pre-v1.12.7 the admin endpoint happened to be reachable by anyone with any admin flag, so the bug went unnoticed; post-v1.12.7 a delegated user without can_manage_connections started getting 403 Forbidden on the lookup, the .catch(() => undefined) swallowed the error, and the tile rendered the protocol (RDP) instead of the actual connection name (e.g. cicsazt1mgt-t). The bug was entirely cosmetic — sessions still connected, audit logs still recorded the correct connection ID, file transfer still worked — only the visual label on the tile was wrong, and only for users in custom delegated roles. v1.12.8 swaps the call to getMyConnections() (the user-scoped /api/user/connections endpoint), which returns the same Connection[] shape, is filtered to the connections the user is actually allowed to reach, and requires no admin permission flag. No backend changes, no migrations, no new environment variables, no new dependencies — two lines in one frontend file plus an explanatory comment.",
    sections: [
      {
        title: "Symptom — delegated users saw 'RDP' on every tile instead of the connection name",
        description:
          "A customer running v1.12.7 with a custom delegated role — call it session-operator, with can_create_connections=true so the user can create personal connections, but with can_manage_connections=false and can_manage_system=false so they cannot administer other users' connections or change global settings — reported that the session tile in the Active Sessions sidebar on the right of the /session/{uuid} page was showing the protocol (RDP) instead of the server name (cicsazt1mgt-t) they had configured. Two screenshots told the whole story: on a can_manage_system=true account the tile correctly read cicsazt1mgt-t with RDP as a subdued sub-label; on the session-operator account the same connection rendered as a tile labelled RDP with no name visible anywhere. The session itself worked perfectly — keyboard, mouse, clipboard, audio, recording, audit, idle timeout, the lot. Opening a second session in a different tab produced a second RDP tile, then a third RDP tile, with no way to tell which tile belonged to which server. The customer noticed the regression specifically because the session-operator role had three sessions open simultaneously (jump-host RDP, target database RDP, target appliance SSH) and needed to switch between them quickly during an incident — the sidebar labels were the primary way they kept the tiles straight.",
      },
      {
        title:
          "Root cause — admin endpoint used for a name lookup that v1.12.7 correctly locked down",
        description:
          "SessionClient.tsx runs a one-shot Phase 1 effect when the route mounts that resolves the connectionId URL parameter into a display name for the tile. The effect issued two parallel HTTP requests: getConnectionInfo(connectionId) → GET /api/user/connections/{id}/info (returns the ConnectionInfo interface, which deliberately does NOT include the name field) and getConnections() → GET /api/admin/connections to fetch the full Connection[] and pluck the matching row's name field. Pre-v1.12.7 step 2 worked for every user with any admin flag because the require_admin middleware was the only check on the endpoint and it admits any user holding has_any_admin_permission() — even users with only can_create_connections=true got the full admin connection list. That was itself the v1.12.7 finding, and v1.12.7 correctly tightened the endpoint with a per-handler check_connection_management_permission(&user)? call. Post-tightening, the session-operator role lost access to step 2. The frontend's .catch(() => undefined) (added defensively to keep the page rendering even if the lookup fails) silently turned the 403 into a connection === undefined value, the connection?.name chain produced undefined, the name: connectionName || protocol.toUpperCase() fallback in the three createSession({...}) call sites fired, and the tile rendered with name = 'RDP'. There was no console error, no toast, no audit-log entry — exactly the silent-fallback pattern the defensive .catch had been written to provide, except now it was firing on the happy path for an entire class of users. This was a pure frontend bug that v1.12.7's backend fix only revealed; the underlying API mismatch (using an admin endpoint to look up data also reachable via a user endpoint) had been in SessionClient.tsx for a long time. Audit confirms no other frontend file makes the same mistake — getConnections() is also called from AdminSettings.tsx, which is correctly admin-only.",
      },
      {
        title: "Fix — swap to the user-scoped getMyConnections() endpoint",
        description:
          "frontend/src/pages/SessionClient.tsx swaps the import and the call from getConnections to getMyConnections. getMyConnections() calls GET /api/user/connections, returns the same Connection[] shape, and is filtered server-side to exactly the connections the user is allowed to launch. For a full admin the response is identical in content to /admin/connections; for a delegated user it is a subset, but always a subset that INCLUDES the connectionId the user is mid-launch on — if it didn't, the preceding getConnectionInfo(connectionId) call would itself have returned 403 and the launch would have been blocked higher up in the flow. So conns.find(...) always resolves on the happy path, and the name field is always present. The three downstream createSession({...}) calls (lines 508, 710, 801 of the same file) are unchanged — they continue to use name: connectionName || protocol.toUpperCase() as the defensive fallback for the genuinely-degenerate case where neither endpoint returned a name (e.g. a connection was deleted between the URL being shared and the page mounting). That fallback now only fires in genuine error paths instead of for an entire user class. A new code comment documents the v1.12.7 interaction so future readers don't recreate the bug. The other call site of getConnections() in AdminSettings.tsx is correct to use the admin endpoint and is unchanged.",
      },
      {
        title: "Acceptance test",
        description:
          "Apply v1.12.8 to a deployment that already has v1.12.7 running. Under Settings → Roles, create a delegated role called session-operator with ONLY can_create_connections=true (no can_manage_system, no can_manage_connections, no can_view_audit_logs, no can_view_sessions). Create a test user, assign them the session-operator role, and grant them access to at least one RDP connection. Sign in as the test user, click the RDP connection from the dashboard, and wait for the session to establish. Expected on v1.12.8: the tile in the Active Sessions sidebar shows the connection's configured name (e.g. cicsazt1mgt-t) with the protocol (RDP) as a subdued sub-label. Was on v1.12.7: the tile shows only RDP. Open the browser devtools network panel and reload the page — the page should issue a GET /api/user/connections (200) request and NO GET /api/admin/connections request. Sign in as a full admin and repeat — the tile label is unchanged from v1.12.7 because the admin's /api/user/connections response already includes every connection in the system. Open three different connections in three browser tabs as the session-operator user — the sidebar should now show three distinct names, one per tile, instead of three identical RDP labels. No DB migrations, no new env vars, no new Cargo or npm dependencies, no backend code changes — the v1.12.7 RBAC tightening on GET /api/admin/connections is preserved.",
      },
    ],
  },
  {
    version: "1.12.7",
    subtitle:
      "Patch release — Security: per-handler RBAC on /api/admin/*. A penetration test against v1.12.6 found that the router-level require_admin middleware guarding /api/admin/* is intentionally a coarse gate — it accepts any user holding ANY of the nine admin permission flags. The granular access decision is supposed to live in each handler via a check_*_permission(&user) call from services::middleware. Seventeen handlers were missing that per-handler check, so a delegated user holding only can_view_audit_logs=true could read every system setting, watch any user's recorded session, list every connection target, fingerprint internal service health, dump the full RBAC matrix, or bounce production DMZ links. CSRF and JWT authentication were intact in every case — this was strictly an authorization gap. v1.12.7 adds the appropriate check_*_permission(&user)? call as the first statement of every affected handler. Users with the super-admin can_manage_system flag (the default admin role) see no behaviour change. Custom delegated roles now see 403 Forbidden on endpoints they should never have been able to reach. No DB migrations, no new env vars, no new dependencies.",
    sections: [
      {
        title: "Symptom — a SOC-analyst-style account could read every admin page",
        description:
          "A pentester audited the /api/admin/* surface of a v1.12.6 deployment under a custom delegated role that held a single admin flag — can_view_audit_logs=true, with every other admin flag (can_manage_system, can_manage_users, can_manage_connections, can_view_sessions, etc.) explicitly off. The intent of that role was 'a SOC analyst account that can read the audit log and nothing else'. The pentester observed that the same account could call GET /api/admin/settings and receive the full system settings table including SMTP server, AD bind DN, Vault address, DNS zones, auth method toggles; GET /api/admin/recordings and GET /api/admin/recordings/{id}/stream to list and play back any user's recorded RDP/VNC/SSH session; GET /api/admin/health to fingerprint internal DB / guacd / Vault reachability, ClamAV engine version, signature freshness; GET /api/admin/connections, GET /api/admin/connection-folders, GET /api/admin/tags, GET /api/admin/connection-tags to enumerate every connection target the deployment manages; GET /api/admin/roles, GET /api/admin/roles/{id}/mappings to dump the full RBAC matrix; GET /api/admin/certs to read the TLS/mTLS certificate inventory with fingerprints and expiry windows; POST /api/admin/dmz-links/reconnect to bounce production DMZ links; PUT /api/admin/safeguard/config to rotate the Safeguard JIT integration API key; POST /api/admin/safeguard/test to probe arbitrary Safeguard appliances with a body-supplied secret; GET /api/admin/metrics to read host CPU / memory / capacity estimates; PUT /api/admin/connection-folders/{id} to rename folders; and DELETE /api/admin/tags/{id} to delete tags. The CSRF token and JWT requirement were intact in every case — this was strictly an authorization-gap finding, not an authentication or CSRF bypass.",
      },
      {
        title: "Root cause — require_admin is a coarse gate; per-handler checks were missing",
        description:
          "/api/admin/* is layered with three middlewares in routes/mod.rs: require_csrf verifies the CSRF double-submit cookie, require_auth verifies the JWT and injects AuthUser via Extension, and require_admin confirms the user holds ANY of the nine admin permission flags via AuthUser::has_any_admin_permission(). require_admin is by design a coarse gate — it answers 'is this user an admin surface user at all?' so a custom role with a single admin flag can still reach the subset of pages that flag covers. The granular access decision is then the responsibility of each handler, which must call one of the helpers in services::middleware: check_system_permission(&user)? (accepts can_manage_system); check_user_management_permission(&user)? (accepts can_manage_system or can_manage_users); check_connection_management_permission(&user)? (accepts can_manage_system or can_manage_connections); check_audit_permission(&user)? (accepts can_manage_system or can_view_audit_logs); check_session_permission(&user)? (accepts can_manage_system or can_view_sessions). can_manage_system is the super-admin flag and short-circuits every check above. About fifty-three of the ~70 admin handlers do call one of these helpers as their first statement. The seventeen identified by the pentest do not. Some never took Extension(user) at all (e.g. list_recordings); others took it for use in audit-log fields but never gated on a permission flag.",
      },
      {
        title: "Fix — add the missing check to every affected handler",
        description:
          "v1.12.7 adds the appropriate crate::services::middleware::check_*_permission(&user)?; call as the first statement of every affected handler, matching the pattern already used by the ~53 other admin handlers in the same files. Endpoints requiring can_manage_system: settings (GET), settings/sso/test, roles (GET), roles/{id}/mappings, metrics, safeguard/config (PUT), safeguard/test, health, certs, dmz-links/reconnect. Endpoints requiring can_manage_connections: connections (GET list), connection-folders (GET + PUT), tags (GET + DELETE), connection-tags (GET). Endpoints requiring can_view_sessions: recordings (GET + stream). The require_admin middleware itself is unchanged because it correctly models the coarse 'is this an admin surface?' question; tightening it would break legitimate delegated roles that should still see the subset of pages their single flag covers. Handlers that previously had no Extension(user) parameter at all now take one — six handlers in admin.rs (test_sso_connection, list_roles, get_role_mappings, list_connection_folders, update_connection_folder, get_metrics, list_admin_tags, delete_admin_tag, list_admin_connection_tags), one in safeguard.rs (test_connection), two in admin/recordings.rs (list_recordings, stream_recording), one in admin/dmz.rs (reconnect_links), one in health.rs (service_health, which also had to switch from Json<ServiceHealth> to Result<Json<ServiceHealth>, AppError>), and one in certs.rs (list_certs, ditto).",
      },
      {
        title: "Operator action — audit custom delegated roles",
        description:
          "If you only use the default admin role (which holds every flag), no action is required — your admin users see no behaviour change. If you maintain any custom delegated admin roles, audit them under Settings → Roles: for each role confirm that the holder needs the flag matching each endpoint they were previously able to reach (the table in the CHANGELOG is the authoritative mapping), grant the missing flag if the access is legitimate, or remove the flag the role previously over-relied on if the access was accidental. Roles holding only can_view_audit_logs continue to access the audit log surface (GET /api/admin/audit-logs, GET /api/admin/audit-logs/export, etc.) exactly as before — those handlers always had the correct per-handler check. The HTTP response body for 403 Forbidden is the standard AppError::Forbidden shape — {'error':'Forbidden'} with status 403 — and the frontend admin pages already render a sensible 'you don't have permission' state from that response.",
      },
      {
        title: "Acceptance test",
        description:
          "Create a delegated role with only can_view_audit_logs enabled and assign it to a test user. Have the test user sign in and call GET /api/admin/audit-logs — must return 200 OK on v1.12.7, confirming the legitimate audit-log access is preserved. Call GET /api/admin/settings — must return 403 Forbidden on v1.12.7 (returned 200 OK on v1.12.6). Call GET /api/admin/recordings — must return 403 Forbidden. Call GET /api/admin/health — must return 403 Forbidden. Add can_manage_system=true to the same role, have the user sign out and back in (the JWT carries the permission snapshot), and all endpoints above must now return 200 OK.",
      },
    ],
  },
  {
    version: "1.12.6",
    subtitle:
      "Patch release — Security: OIDC RP-Initiated Logout. A penetration test against v1.12.5 found that clicking Log out in the Strata SPA cleared Strata's own session cookies and revoked its local JWT, but never contacted the IdP. The browser kept the upstream Keycloak SSO cookie, so the very next click of Sign in silently re-authenticated against the surviving Keycloak session without prompting for a password or MFA challenge. v1.12.6 wires up the standard OpenID Connect RP-Initiated Logout 1.0 flow: POST /api/auth/logout now returns a post_logout_url field when the user signed in via SSO, the SPA navigates the browser to it, and Keycloak destroys its own session before bouncing back to /login via the registered post-logout redirect URI. One IdP configuration step is required (register https://<your-strata-fqdn>/login under Valid Post Logout Redirect URIs); see the operator section below. No migrations, no new environment variables, no new Cargo or npm dependencies.",
    sections: [
      {
        title: "Symptom — Log out, then Sign in again, no password or MFA prompt",
        description:
          "A pentester signed in to a v1.12.5 deployment via the Keycloak SSO button on the login page, completed the password and MFA challenge, was redirected back to the dashboard, and then clicked Log out in the top-right menu. The SPA cleared its in-memory user state and navigated to /login. So far so good. The pentester then immediately clicked Sign in with Keycloak on the login page — and was redirected straight back to the dashboard, without ever being asked for a password or MFA token. The whole sequence — sign out, sign back in — took two clicks and no keyboard input. From a workstation-walk-away threat model this is the same as never having logged out at all: a co-worker who sits down at the abandoned session can re-enter the application as the previous user in two clicks. The audit log shows two separate auth.sso_login rows, which correctly reflects what Strata saw on the wire, but is misleading about what the user did at the keyboard — they typed nothing.",
      },
      {
        title: "Root cause — Strata clears its own cookies; the IdP cookie survives",
        description:
          'POST /api/auth/logout did exactly two things: (1) revoked the locally-signed access and refresh JWTs in the token_revocations table (so a stolen Strata token could not be replayed), and (2) tombstoned the four session cookies (access_token, refresh_token, csrf_token, session_expires) with Set-Cookie: ...; Max-Age=0. What it did NOT do was contact Keycloak. Keycloak\'s own SSO session cookie (set on the Keycloak domain, not the Strata domain, by the authorization-code flow during the original sign-in) is invisible to Strata\'s logout code path and survived untouched. The next click of Sign in with Keycloak re-ran the authorization-code flow, hit Keycloak\'s authorize endpoint, observed the live SSO cookie, and short-circuited the password and MFA prompt — exactly the flow Keycloak exists to implement. The existing comment in backend/src/routes/auth.rs acknowledged the gap explicitly: "OIDC tokens are not revoked here — they should be ended at the IdP via the configured end-session endpoint. The cookie clear below is still best-effort for the browser side." v1.12.5 simply never built out that "should be ended at the IdP" path.',
      },
      {
        title: "Fix — capture id_token, build IdP logout URL, navigate the browser",
        description:
          "Three coordinated changes wire up OpenID Connect RP-Initiated Logout 1.0 end to end. (1) The SSO callback now persists the raw id_token in a new HttpOnly; Secure; SameSite=Strict; Path=/api cookie alongside the existing access / refresh / csrf / session_expires set. Lifetime matches the refresh token so the hint survives access-token rotation. (2) POST /api/auth/logout reads the id_token cookie, decodes the unverified iss claim (safe because the token was cryptographically verified at SSO callback time and the cookie is locked behind HttpOnly + Secure + SameSite=Strict + Path=/api), looks up the matching sso_providers row by issuer_url, fetches the provider's OIDC discovery document (cached in services::auth), and — if the IdP advertises end_session_endpoint — builds the RP-initiated logout URL and returns it as a new post_logout_url field on the JSON response. (3) The frontend handleLogout awaits the apiLogout result; when post_logout_url is present it issues window.location.assign(...) so the browser navigates to Keycloak, which destroys its SSO session cookie for the client and bounces back to /login via the registered post_logout_redirect_uri. Local state is wiped synchronously so the UI snaps back instantly.",
      },
      {
        title: "Defence in depth — the id_token cookie is tombstoned on every logout",
        description:
          "The new id_token cookie is tombstoned (Set-Cookie: id_token=; Max-Age=0; HttpOnly; Secure; SameSite=Strict; Path=/api) on every logout — including local-account logouts where no id_token was ever set — so a stale value never leaks across sessions on a shared workstation. The id_token cookie itself is HttpOnly so SPA JavaScript can never read it; Path=/api so it is only sent on calls to the backend; and SameSite=Strict so a third-party site cannot trigger a cross-site request that exfiltrates it. The new OidcDiscovery.end_session_endpoint field deserialises with serde(default), so legacy IdPs that omit end_session_endpoint from their discovery document continue to parse and the backend simply falls back to the local-only logout path (logged at warn level the first time it can't build the URL).",
      },
      {
        title: "Operator action — register the post-logout redirect URI",
        description:
          'Required in Keycloak ≥ 25.0 (which validates Valid Redirect URIs and Valid Post Logout Redirect URIs separately) and recommended on every IdP for clarity: open the Keycloak admin console → your realm → Clients → the Strata client → Settings → scroll to Access settings → add https://<your-strata-fqdn>/login to Valid Post Logout Redirect URIs and Save. Wildcards are allowed by Keycloak; the explicit path is recommended for least-privilege. Without this entry, Keycloak rejects the logout redirect with "Invalid redirect uri" and the user lands on a Keycloak error page instead of /login. The user IS still logged out of both Strata AND Keycloak at that point (the redirect rejection happens AFTER the session is destroyed) — they just see a confusing error page instead of the Strata Login screen. Auth0, Okta, Entra ID, and other RP-Initiated Logout 1.0 conformant IdPs have the same configuration knob under different names (Auth0: Allowed Logout URLs; Okta: Sign-out redirect URIs; Entra ID: Front-channel logout URL plus post_logout_redirect_uri validation through Redirect URIs). Register https://<your-strata-fqdn>/login under whichever field your IdP exposes.',
      },
      {
        title: "Tests — twelve new cases pin the fix",
        description:
          "Three new unit tests in backend/src/services/auth.rs (oidc_discovery_deserializes_with_end_session_endpoint covers the Keycloak-shape document, oidc_discovery_deserializes_without_end_session_endpoint covers legacy IdPs that omit the field, and the existing oidc_discovery_clone is extended to cover the new field). Seven new unit tests in backend/src/routes/auth.rs (logout_response_tombstones_id_token_cookie pins the cookie tombstone; logout_response_includes_post_logout_url_field pins the JSON response shape including the field being null for local logouts; id_token_issuer_extracts_iss_from_jwt, id_token_issuer_returns_none_for_malformed_input, and id_token_issuer_returns_none_when_iss_claim_missing pin the defensive iss-claim parsing; build_rp_initiated_logout_url_returns_none_without_id_token_cookie and build_rp_initiated_logout_url_returns_none_without_db pin the helper's short-circuit branches). Two new vitest cases in frontend/src/__tests__/api.test.ts pin the new LogoutResponse shape (returns parsed body including post_logout_url when backend supplies it) and back-compat with v1.12.5 backends that return an empty body (returns null when the response body cannot be parsed). Every pre-existing logout, SSO callback, and OIDC discovery test continues to pass without modification.",
      },
      {
        title: "Operator impact",
        description:
          "No migrations, no environment variables added, no Cargo or npm dependencies added, no new images or containers. Recommended deploy: docker compose pull && docker compose up -d --build from v1.12.5 (or the ghcr / k8s equivalent). One IdP-side configuration step is required (register https://<your-strata-fqdn>/login under Valid Post Logout Redirect URIs); see the previous section. IdPs that omit end_session_endpoint from their OIDC discovery document fall back transparently to the pre-fix behaviour. The new id_token cookie is approximately 1–2 KB on a typical Keycloak deployment — well inside the 4 KB per-cookie browser limit. Realms with unusually large id_tokens (custom claims pushing past 4 KB) should review their client-scope mappers; nothing in Strata caps the size. Local-account-only deployments are unaffected: the logout response carries post_logout_url: null and the SPA takes the local /login path exactly as before.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      {
        title: "Symptom — post-approval tunnels rejected with 'invalid credentials'",
        description:
          "User maps to a credential profile of kind='safeguard' whose Safeguard account requires Approver or Reviewer action (common policy on tier-0 / privileged accounts). User submits the request via Credentials → Bulk Checkout with a justification, SPA shows 'Awaiting approver — request {rid} is queued in Safeguard (state: PendingApproval)', approver acts, SPA's polling loop hits POST /api/user/safeguard/release, row flips to Released with the appliance-reported TTL. User clicks the affected connection on the Sessions page — tunnel opens to guacd, guacd attempts the RDP/SSH handshake against the target, target rejects, frontend renders 'Connection Error — Authentication failure (invalid credentials?)'. The user can confirm the password itself is fine: (a) Safeguard portal shows the account checked out with a still-live TTL; (b) manually copying the password from the portal and typing it into the in-band auth prompt always works; (c) the target AD account is never locked out despite repeated failures, proving the credential reaching the target is structurally wrong, not just incorrect.",
      },
      {
        title: "Root cause — release_pending returned username=None",
        description:
          "Safeguard's REST surface only echoes AccountName back at request-creation time (POST /service/core/v4/AccessRequests response). Subsequent POST /AccessRequests/{id}/CheckoutPassword calls return only a JSON-encoded plaintext string — no AccountName, no AccountDomainName. The initial jit_checkout orchestrator captures account_name from the creation response and threads it through CheckoutResult.username → password_cache::store(...) → safeguard_cached_passwords.username, so auto-released accounts work correctly. But the post-approval release path is structurally different: the original bulk-checkout call returned JitOutcome::PendingApproval { request_id, … } and the SPA polled POST /api/user/safeguard/release until the approver acted; that polling endpoint dispatches through services::safeguard::release_pending(...), which only has the request_id to work with and (until v1.12.3) hard-coded username: None on the Released arm of its match (with an in-code TODO comment acknowledging 'the caller is expected to use the profile's stored username or refetch the request details' — neither happened). The route handler release_safeguard_pending faithfully forwarded that None into password_cache::store(...), persisting the cache row with username=NULL. The credential_profiles table has no username column for kind='safeguard' rows (Safeguard's AccountName is the only source of truth), so there was no fallback. At tunnel-open time, routes/tunnel.rs loaded the cache row, assigned safeguard_username = cached.username (= None), and the tunnel handler propagated (None, Some(<correct-password>)) into the (vault_username, vault_password) tuple — guacd/FreeRDP/OpenSSH then attempted authentication with an empty NLA username and the correct password, which the target rejected pre-lockout.",
      },
      {
        title: "Fix — get_access_request_status refetches AccountName",
        description:
          "A new client::get_access_request_status(...) function (renamed and extended from the v1.12.x-era get_access_request_state(...) helper) issues a GET /service/core/v4/AccessRequests/{id} and now returns AccessRequestStatus { state: Option<String>, account_name: Option<String> } instead of a bare Option<String>. The state field is what the existing password_cache::check_request_validity(...) validator was already using to decide whether a cached row was still live (PasswordCheckedOut → Active, anything else → Inactive, 404 → Inactive). The new account_name field is consumed by the Released arm of release_pending: Ok(Some(status)) → status.account_name; Ok(None) [404 — purged] → None; Err [appliance hiccup] → None and logged at warn. The username is then plumbed through into CheckoutResult.username; the route handler release_safeguard_pending is unchanged and already forwarded outcome.username into password_cache::store(...), so the cache row now carries the real username for both paths. The single existing caller of get_access_request_state (check_request_validity) was updated to read status.state and map the 404 case explicitly onto CacheValidity::Inactive, preserving prior implicit behaviour. Refetch failure is non-fatal — the password is still returned and cached; the only consequence of a failed refetch is the pre-fix behaviour. The safeguard_checkout_audit row records the success outcome before the refetch runs, so the audit trail is unaffected by refetch outcome.",
      },
      {
        title: "Backwards compatibility — zero surface change",
        description:
          'Unchanged: POST /api/user/safeguard/bulk-checkout request and response shapes, POST /api/user/safeguard/release request and response shapes, GET /api/user/safeguard/cached row shape, safeguard_cached_passwords table schema, safeguard_checkout_audit table schema, safeguard_config table schema. No migrations introduced. No environment variables added. No Cargo or npm dependencies added. Cache rows persisted by v1.12.2 against approval-required accounts will continue to carry username=NULL until they expire (default TTL: the profile\'s own ttl_hours); operators who want to evict them immediately can click Check in all in the bulk-checkout card or call POST /api/user/safeguard/checkin with {"profile_ids": []}. After the rebuild, a fresh approve-and-release cycle will write a complete row.',
      },
      {
        title: "Why was this not caught earlier?",
        description:
          "Two reasons. (1) Most lab and dogfood Safeguard accounts auto-release — the integration test plan exercised the jit_checkout → Released happy path extensively (which DOES populate username correctly from the CreateAccessRequest response), but the PendingApproval → poll → Released path requires an approver-in-the-loop setup the lab appliance was not configured for at integration time. (2) The cache row looked superficially complete — all other fields (expires_at, request_id, ciphertext, encrypted_dek, nonce) were populated correctly, so the cache validator (check_request_validity) reported Active against the live appliance request and the tunnel handler took the 'cache hit' branch happily. The failure surfaced only at the very last hop (guacd → target), where the empty NLA username triggered a generic auth-failure response that looked like every other 'wrong password' failure to a harried operator. The frontend toast text — 'Authentication failure (invalid credentials?)' — surfaced unchanged from guacd's own in-protocol error, so the SPA had no signal that the issue was a missing username rather than a wrong password.",
      },
      {
        title: "Operator impact",
        description:
          "No migrations. No new environment variables. No config changes. No new Cargo or npm dependencies — net zero supply-chain churn. Recommended deploy: docker compose pull && docker compose up -d --build backend (backend only; the frontend image rebuilds with the bumped __APP_VERSION__ so this card surfaces on first sign-in after upgrade, but the frontend JS/CSS bundle is otherwise byte-identical to v1.12.2). Sites that do not use Safeguard JIT credential profiles can defer indefinitely — the fix is entirely scoped to the services::safeguard::* module tree. Sites using only auto-released Safeguard accounts (no approver-gated entitlements) are also unaffected in practice but should still upgrade in normal cadence; the fix hardens the post-approval path against the same regression returning under future appliance-side workflow changes. The strata-dmz relay binary cosmetically bumps version (shared workspace [workspace.package].version field) but its wire protocol and behaviour are byte-identical to v1.12.2, so mixed v1.12.2 / v1.12.3 deployments handshake cleanly; the Admin → DMZ Links tab will show a Mixed indicator until every link is upgraded.",
      },
    ],
  },
  {
    version: "1.12.2",
    subtitle:
      "Patch release — operational polish on the v1.11.0 outbound-share landing and the v1.12.0/v1.12.1 AV pipeline. Thirteen merged PRs (#268–#280) roll into one bundle covering six themes: five day-to-day UX fixes on the Outbound Quick Share surface (paste-and-run snippets via a new File path input, visibility-gated 60s auto-refresh on the admin Outbound Shares tab, curl snippets always print a completion summary so a stale token is distinguishable from a transport error, justification textarea stays visible when drive redirection is disabled by group policy, ConfirmModal renders via a React portal so the backdrop covers the full viewport); a long-overdue email fan-out to approvers when a non-bypass outbound share lands in the queue (new OutboundSharePending template, with follow-up fixes for the roles.can_manage_system join and the requester_id self-exclusion); a tenant-URL resolution chain (services::settings::tenant_base_url) that resolves system_settings → BASE_URL env var → https://strata.local so email links work on every install instead of just admin-configured ones; a clamav sidecar healthcheck pinned to 127.0.0.1 to dodge IPv6 localhost resolution on dual-stack hosts (was reporting unhealthy despite serving scans); a recordings-volume permission fix that lets the session-recording sweeper actually prune expired .guac files on bind-mounted setups; and a Kali Rolling VDI image (contrib/vdi-kali) with the kali-linux-large metapackage for security teams who want a tunnel-routed jump-box for authorised offensive engagements.",
    sections: [
      {
        title: "Outbound Quick Share — paste-and-run snippets via File path input",
        description:
          "The snippet builder under Outbound Share → Generate upload command gains a new File path text input between the snippet-format dropdown and the Skip TLS verification checkbox. The user pastes the path of the file they intend to upload (e.g. C:\\Users\\analyst\\Desktop\\report.pdf for PowerShell, /home/analyst/report.pdf for curl) and the snippet body is regenerated with the path substituted in place of the <your-file> placeholder, using format-aware quoting so the resulting snippet is paste-and-run instead of paste-edit-run: POSIX curl single-quoted with embedded ' escaped as '\\'', curl.exe on Windows double-quoted via the full CommandLineToArgvW rule (backslash runs before a \" doubled, backslash runs at end-of-string doubled, then \"…\" wrapped), PowerShell 7 single-quoted with embedded ' doubled (''). Empty input keeps the literal <your-file> placeholder so the snippet stays self-documenting; the helper text below the snippet flips between \"Paste this into a shell inside the session\" and \"This snippet uploads <the supplied path>\" so the intent is unambiguous. The CommandLineToArgvW handler closes a CodeQL js/incomplete-sanitization alert that a single-pass replace(/\"/g, '\\\"') would have tripped.",
      },
      {
        title: "Outbound Quick Share — admin tab auto-refresh while visible",
        description:
          "The admin Outbound Shares tab now refreshes the pending queue and the history list every 60s while the browser tab is visible, so approvers can leave the tab open and watch new submissions land without F5. The poll is suspended on document.visibilitychange → hidden (a parked tab on a laptop does not keep the backend warm overnight) and resumed with one immediate refresh on visibilitychange → visible (the tab is up-to-date the instant the operator returns to it). No change to the underlying admin API contract — the polling effect simply re-fires the existing loadPending() and loadHistory() calls that the initial mount already runs.",
      },
      {
        title: "Outbound Quick Share — curl snippets always print a completion summary",
        description:
          "Every shipped curl / curl.exe snippet variant now ends with a printf-style summary block that echoes the HTTP status, the response body, and a one-line interpretation: 'Upload succeeded', 'Token rejected (probably expired or already used)', or 'Backend rejected the file — see body above'. Before this fix a stale ingest token returned 400 Bad Request with a JSON error body and curl exited 0 (HTTP-error responses are still \"successful HTTP transactions\" by curl semantics), so the user saw nothing on the terminal and had no way to tell the snippet had been rejected. The summary block runs unconditionally whether the upload succeeded or failed. Also: the justification textarea now stays visible when drive redirection is disabled by group policy (the HTTPS upload-command flow needs a justification too, since it is part of the bound state on the minted token), and the ConfirmModal renders into a document.body-anchored React portal so the backdrop covers the full viewport instead of being clipped by an ancestor overflow:hidden.",
      },
      {
        title: "Outbound-share approver email fan-out",
        description:
          "Until v1.12.2, outbound submissions that landed in the approval queue had no email notification path — only the in-app PendingApprovalWatcher popup. A new transactional email event (OutboundShareEvent::Pending) now fires the moment a non-bypass outbound submission lands in the queue. The new outbound_share_pending.mjml / .txt.tera template pair joins the existing four CheckoutEvent templates under the same Tera + mrml + Outlook dark-mode VML wrapper pipeline, and the same opt-out / audit rules apply: per-user notifications_opt_out suppresses delivery and writes a notifications.skipped_opt_out audit row, the retry-after-failure worker re-renders and re-sends transient failures up to three times, and the rendered body is never persisted into email_deliveries. Two follow-up fixes hardened the fan-out: the approver-discovery SQL now joins roles.can_manage_system (the column does not exist on users — super-admins were silently excluded from the fan-out on early dogfood), and the self-exclusion now reads requester_id off the Pending event payload (was reading the wrong field, so submitters who held the approver bit emailed themselves about their own request).",
      },
      {
        title: "BASE_URL fallback for transactional email links",
        description:
          "Every transactional email template that includes a link back into the SPA (checkout request / approval / rejection / outbound-share pending) needs a tenant URL to build absolute hrefs. Until now the templates rendered https://strata.local as the link target whenever the system_settings.tenant_base_url admin setting was empty — broken even on perfectly-healthy installs that simply hadn't filled in the optional admin field. A new services::settings::tenant_base_url(pool) helper resolves the tenant URL in three deterministic tiers: system_settings.tenant_base_url (admin-set, takes precedence when non-empty) → BASE_URL environment variable (deployment-set, picked up on every dispatch with no restart required) → https://strata.local (build-in last-resort that at least produces a syntactically valid URL). All four email-render call sites (notifications::build_context, notifications::build_outbound_share_context, email::worker rebuild path, routes/notifications::sample_context preview) share the helper so the resolution order is identical regardless of whether the email was rendered on the originating dispatch, on a retry, or in the admin preview.",
      },
      {
        title: "ClamAV sidecar healthcheck pinned to 127.0.0.1",
        description:
          "The Docker HEALTHCHECK on the clamav sidecar previously ran clamdscan --ping localhost, which resolved to ::1 on hosts with dual-stack /etc/hosts (most modern Linux distributions, including Ubuntu 22.04+ and Debian Bookworm) and immediately failed because clamd only binds IPv4 (TCP 0.0.0.0:3310) inside the container. The sidecar was perfectly healthy and serving scans from the adjacent backend container, but Docker reported unhealthy every minute, which poisoned every health-aware orchestrator that read the container status (docker-compose ps, docker events, Kubernetes liveness, anything that polled /v1.41/containers/{id}/json). Pinning the healthcheck to 127.0.0.1 explicitly avoids the resolver ambiguity and the sidecar now reports healthy from boot. Operators on the av profile pick up the fix with docker compose --profile av up -d --build clamav; the clamav-db named volume that holds the signature DB is preserved across the rebuild.",
      },
      {
        title: "Recordings volume write permission + Kali Linux VDI image",
        description:
          "The strata user inside the backend container could not delete expired .guac recordings from /var/lib/strata/guac-recordings on hosts where the volume was bind-mounted from a directory owned by root:root (the default when docker-compose creates the directory on first bring-up). The session-recording sweeper would silently fail to prune expired recordings, growing the directory until the operator noticed and chown'd it by hand. The backend Dockerfile now mkdirs the directory with the correct UID:GID baked into the image, and the entrypoint script chowns the bind-mount contents on startup when writable, so the sweeper succeeds on every tick. Separately: a new contrib/vdi-kali/ profile ships a Kali Rolling VDI desktop with the kali-linux-large metapackage preinstalled (Nmap, Metasploit Framework, Burp Suite Community, Wireshark, Aircrack-NG, Hashcat, John, sqlmap, Hydra, and the rest of the standard toolkit) targeted at security teams who want a clean, audited, tunnel-routed jump-box for authorised offensive engagements. Built the same way as contrib/vdi-sample (docker build contrib/vdi-kali) and tunnel-in via the existing VDI flow; runs with the same per-session network isolation as every other VDI image.",
      },
      {
        title: "Operator impact",
        description:
          "No migrations. No new environment variables (BASE_URL is read by the new tenant_base_url helper, but it is an existing variable documented since v1.7.0). No new Cargo or npm dependencies — net zero supply-chain churn across the thirteen PRs. Recommended deploy: docker compose pull && docker compose up -d --build. Sites using the bundled AV sidecar should add --profile av to pick up the rebuilt clamav image with the healthcheck fix. Sites adopting Kali VDI should docker build contrib/vdi-kali separately before bringing the new VDI online. The strata-dmz relay binary cosmetically bumps version (it shares the workspace [workspace.package].version field) but its wire protocol and behaviour are byte-identical to v1.12.1, so mixed v1.12.1 / v1.12.2 deployments handshake cleanly; the Admin → DMZ Links tab will show a Mixed indicator until every link is upgraded.",
      },
    ],
  },
  {
    version: "1.12.1",
    subtitle:
      "Patch release — operational polish on the v1.12.0 antivirus landing. Fourteen merged PRs roll into one bundle covering five themes: a new Admin → Health → AV card showing backend, daemon version, signature DB versions, last freshclam update, and last-30d verdict tally; friendly classified error messages on every AV block (timeout / transport / missing-signatures / generic, instead of the raw engine spew); live upload progress everywhere (browser-side percentage bar on the upload toast, indeterminate 'Awaiting AV scan' bar on every pending row in MY SUBMISSIONS, and terminal-side --progress-bar + Write-Progress meters on the curl and PowerShell snippets); a new Admin → AV-Blocked Files tab unifying inbound and outbound blocks behind one audit query; and scanner-side correctness fixes (hourly freshclam with forced clamd reload after each update, scan-size default raised to 500 MiB to match the upload cap, STRATA_AV_* env-var wiring corrected, super-admin role check made uniform across all three outbound routes).",
    sections: [
      {
        title: "Scanner health visible without docker exec",
        description:
          "New Admin → Health → AV card surfaces the active backend (off / clamav / command), a reachability ping, the clamd daemon version, signature DB versions for main / daily / bytecode with their signature counts and update dates, last freshclam timestamp, and a last-30d verdict tally (clean / infected / skipped / error). Degrades gracefully when the daemon is temporarily unreachable — keeps the last known-good values plus a 'stale since HH:MM:SS' badge rather than wiping the panel. Backed by a tiny services::av_health module that caches the handshake for 30 s.",
      },
      {
        title: "Friendly user-facing error messages on AV blocks",
        description:
          "Verdict::user_facing_block_message() in backend/src/services/av.rs classifies AV-block responses by error shape and returns a deterministic actionable message instead of the raw engine output. Infected → 'File rejected by malware scan: <signature>'. Timeout → 'Antivirus scan timed out after Ns; try a smaller file or retry shortly.' Transport (refused / reset / unreachable) → 'Antivirus scanner unreachable; please retry shortly.' Missing-signatures → 'Antivirus signature database not yet ready; please retry shortly.' Generic → the engine's raw text, passed through verbatim. The audit row always carries the unredacted message for forensic review; only the user-facing response is normalised.",
      },
      {
        title: "Live upload progress on every surface",
        description:
          "Browser-side: a new typed xhrUploadJson<T>() helper in frontend/src/api.ts wraps XMLHttpRequest (the only browser API that exposes upload.onprogress) and threads optional UploadProgressOptions through submitOutboundShare / uploadQuickShareFile / the drive-channel finalize helper. Toasts gained an optional progress prop so the inbound and outbound drag-drop flows now drive a single in-place bar that walks 'Uploading 42%' → 'Scanning…' (indeterminate) → final verdict. MY SUBMISSIONS pending rows render an indeterminate 'Awaiting AV scan' bar so the scan + approval wait window is visible. Terminal snippets: curl gains --progress-bar plus -H 'Expect:' (without disabling Expect/100-continue, the server returns 400 on a stale token before the body is read and the meter has nothing to draw); the PowerShell snippet is rewritten from Invoke-WebRequest -Form (no upload progress) to a streaming HttpClient + MultipartFormDataContent + StreamContent pipeline with a Write-Progress poll loop driven by $stream.Position.",
      },
      {
        title: "Admin → AV-Blocked Files unified audit view",
        description:
          "New tab in the admin area surfaces every blocked upload — inbound (file.av_blocked) and outbound (outbound_share.requested with av_status in 'infected' or 'error') — in one paginated, filterable list. Columns: timestamp, direction, filename, byte size, signature or error message, engine, actor, session context. The pre-existing outbound_share.requested audit-event payload was extended with av_status / av_signature / av_backend / direction: 'outbound' / filename keys so both ingest paths join through one action_type filter. Filterable by date range, engine, direction, and signature substring; cursor-paginated at 50/page. Visible to can_manage_system only.",
      },
      {
        title: "Scanner-side correctness fixes",
        description:
          "freshclam cadence bumped to hourly with a forced clamd RELOAD after every successful update — fresh signatures previously sat unused until the next sidecar restart, so the audit trail could record Skipped/error against files the engine had signatures for but hadn't loaded yet. STRATA_AV_MAX_SCAN_SIZE default raised from 100 MiB to 500 MiB to match the Quick Share upload cap — previously a 200 MiB upload landed in the file store unscanned (tagged Skipped/oversize), defeating the v1.12.0 design. STRATA_AV_* env-var wiring corrected so all seven variables documented in .env.example actually take effect (one was silently ignored on v1.12.0 due to a naming drift between Config::from_env and the bootstrap code). Quick Share role-permission check made strictly uniform across the three outbound routes (mint / submit / token-ingest) — a super-admin without can_use_quick_share_outbound is now correctly rejected with 403 on every path, matching the v1.11.0 design intent. CVD / CLD version-parser unit tests now use real on-wire fixtures captured from live clamd.",
      },
      {
        title: "Operator impact",
        description:
          "No migrations. No new environment variables. No config changes. No new Cargo or npm dependencies. The clamav sidecar entrypoint script was updated (cadence + forced reload) so recreate it if you're using the bundled profile: docker compose --profile av up -d --build clamav. Existing STRATA_AV_MAX_SCAN_SIZE overrides continue to apply — the bump is a default change, not a forced value; pin STRATA_AV_MAX_SCAN_SIZE=104857600 in .env to preserve the 100 MiB ceiling. AV-Blocked Files admin tab visible to can_manage_system only. AV Health card surfaces on the existing Admin → Health page. Recommended deploy: docker compose --profile av up -d --build (backend + frontend + clamav containers).",
      },
    ],
  },
  {
