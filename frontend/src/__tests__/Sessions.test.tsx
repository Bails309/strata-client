...
vi.mock("../api", () => ({
  ...
  buildRecordingStreamUrl: vi.fn(
    (id: string) => `wss://localhost/api/admin/recordings/${id}/stream`
  ),
  buildMyRecordingStreamUrl: vi.fn(
    (id: string) => `wss://localhost/api/user/recordings/${id}/stream`
  ),
}));
...
describe("Sessions", () => {
  ...
  it("formats short durations correctly", async () => {
    vi.mocked(getRecordings).mockResolvedValue([makeRecording({ duration_secs: 45 })]);
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime });
    await act(async () => {
      renderSessions();
    });
  ...
  it("formats minutes-only durations", async () => {
    vi.mocked(getRecordings).mockResolvedValue([makeRecording({ duration_secs: 125 })]);
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime });
    await act(async () => {
      renderSessions();
    });
  ...
  it("shows dash for null duration", async () => {
    vi.mocked(getRecordings).mockResolvedValue([makeRecording({ duration_secs: null })]);
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime });
    await act(async () => {
      renderSessions();
    });
