...
export default function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  const launch = useCallback((conn: Connection) => {
    onClose();
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 opacity-40">
          ...
          <input
            ...
            spellCheck={false}
          />
          <kbd className="text-[10px] opacity-30 border rounded px-1.5 py-0.5 font-mono" style={{ borderColor: 'var(--color-border)' }}>ESC</kbd>
        </div>
        ...
        <div ref={listRef} className="max-h-[320px] overflow-y-auto py-1" role="listbox">
          ...
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="opacity-20">
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background: isSelected ? 'var(--color-accent-glow)' : 'rgba(255,255,255,0.05)',
                    color: isSelected ? 'var(--color-accent-light)' : 'var(--color-txt-secondary)',
                  }}
                >
                ...
                {isSelected && (
                  <kbd className="text-[10px] opacity-30 border rounded px-1.5 py-0.5 font-mono shrink-0" style={{ borderColor: 'var(--color-border)' }}>↵</kbd>
                )}
              </div>
            );
          })}
        ...
        {/* Footer */}
        <div className="flex items-center gap-4 px-4 py-2 border-t text-[11px] opacity-30" style={{ borderColor: 'var(--color-border)' }}>
          <span className="flex items-center gap-1"><kbd className="font-mono">↑↓</kbd> navigate</span>
          <span className="flex items-center gap-1"><kbd className="font-mono">↵</kbd> launch</span>
